...
void convertGeodeticEllipsoidToGeocentric(
    ...
    double lat, double lon, double height, double& x, double& y, double& z) {
...
void convertGeocentricToGeodeticEllipsoid(
  ...
  ccsGeodeticEllipsoidToGeocentric.convertTargetToSource(
      &geocentricCoordinates, &geocentricAccuracy, geodeticCoordinates,
      geodeticAccuracy);
...
void convertMslEgm96ToEllipsoidHeight(
    ...
    double lat, double lon, double mslHeight, double& ellipsoidHeight) {
  MSP::CCS::Accuracy sourceAccuracy;
  MSP::CCS::Accuracy targetAccuracy;
  MSP::CCS::GeodeticCoordinates sourceCoordinates(
...
void convertEllipsoidHeightToMslEgm96(
    ...
    double lat, double lon, double ellipsoidHeight, double& mslHeight) {
  MSP::CCS::Accuracy sourceAccuracy;
  MSP::CCS::Accuracy targetAccuracy;
...
void convertGeocentricToUtm(
    MSP::CCS::CoordinateConversionService& ccsGeocentricToUtm, double x,
    double y, double z, long& zone, char& hemisphere, double& easting,
    double& northing) {
  MSP::CCS::Accuracy sourceAccuracy;
  MSP::CCS::Accuracy targetAccuracy;
  MSP::CCS::CartesianCoordinates sourceCoordinates(
  ...
  ccsGeocentricToUtm.convertSourceToTarget(&sourceCoordinates, &sourceAccuracy,
...
std::string convertGeocentricToMgrs(
    MSP::CCS::CoordinateConversionService& ccsGeocentricToMgrs, double x,
    double y, double z, MSP::CCS::Precision::Enum& precision) {
  char* p;
...
int main(int argc, char** argv) {
  ...
  MSP::CCS::UTMParameters utmParameters(
      MSP::CCS::CoordinateType::universalTransverseMercator, 1, 0);
  ...
  //
  MSP::CCS::CoordinateConversionService ccsGeodeticEllipsoidToGeocentric(
      WGE, &ellipsoidParameters, WGE, &geocentricParameters);

  MSP::CCS::CoordinateConversionService ccsGeocentricToGeodeticMslEgm96(
      WGE, &geocentricParameters, WGE, &mslEgm96Parameters);

  MSP::CCS::CoordinateConversionService ccsMslEgm96ToEllipsoidHeight(
      WGE, &mslEgm96Parameters, WGE, &ellipsoidParameters);
  MSP::CCS::CoordinateConversionService ccsEllipsoidHeightToMslEgm96(
      WGE, &ellipsoidParameters, WGE, &mslEgm96Parameters);

  MSP::CCS::CoordinateConversionService ccsGeocentricToUtm(
      WGE, &geocentricParameters, WGE, &utmParameters);
  MSP::CCS::CoordinateConversionService ccsGeocentricToMgrs(
      WGE, &geocentricParameters, WGE, &mgrsParameters);
  ...
  try {
    ...
    convertGeodeticEllipsoidToGeocentric(ccsGeodeticEllipsoidToGeocentric, lat,
                                         lon, height, x, y, z);

    std::cout << "Convert Geodetic (Ellipsoid Height) to Geocentric"
              << std::endl
              << std::endl
    ...
    // conversion
    convertGeocentricToGeodeticEllipsoid(ccsGeodeticEllipsoidToGeocentric, x, y,
                                         z, lat, lon, height);

    std::cout << "Revert Geocentric To Geodetic (Ellipsoid Height): "
              << std::endl
              << std::endl
              << "x: " << x << std::endl
              << "y: " << y << std::endl
    ...
    convertGeocentricToGeodeticMslEgm96(ccsGeocentricToGeodeticMslEgm96, x, y,
                                        z, lat, lon, mslHeight);
    ...
    //
    convertMslEgm96ToEllipsoidHeight(ccsMslEgm96ToEllipsoidHeight, lat, lon,
                                     mslHeight, height);

    std::cout << "Convert Geodetic (MSL EMG96 15M Height) To Geodetic "
                 "(Ellipsoid Height)"
              << std::endl
              << std::endl
              << "Lat (radians): " << lat << std::endl
    ...
    //
    convertEllipsoidHeightToMslEgm96(ccsEllipsoidHeightToMslEgm96, lat, lon,
                                     height, mslHeight);

    std::cout << "Revert Geodetic (Ellipsoid Height) To Geodetic (MSL EGM96 "
                 "15M) Height"
              << std::endl
              << std::endl
              << "Lat (radians): " << lat << std::endl
    ...
    convertGeocentricToUtm(ccsGeocentricToUtm, x, y, z, zone, hemi, easting,
                           northing);
    ...
    mgrsString =
        convertGeocentricToMgrs(ccsGeocentricToMgrs, x, y, z, precision);
    ...
    // Conversion Service
    std::cerr << "ERROR: Coordinate Conversion Service exception encountered - "
              << e.getMessage() << std::endl;

  } catch (std::exception& e) {
    // catch and report any unexpected exceptions thrown
    std::cerr << "ERROR: Unexpected exception encountered - " << e.what()
              << std::endl;
  }
