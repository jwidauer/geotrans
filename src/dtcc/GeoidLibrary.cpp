 ...
 * REUSE NOTES
 *
 *    GeoidLibrary is intended for reuse by any application that requires
 * conversion between WGS84 ellipsoid heights and WGS84, EGM96, or EGM2008
 * orthometric heights.
 *
 ...
 *    first loads NGA's worldwide EGM2008 grid before beginning any
 * interpolations: the FULL-GRID algorithm draws all local interpolation
 * windows' post points directly from this worldwide grid.  On the other hand,
 * if the environment variable is set to "AOI", then the EGM2008 geoid height
 * interpolator uses its AOI-GRID sofware, which loads Area of Interst
 * subregions from NGA's EGM2008 worldwide grid.  Area of Interest support grids
 * cover user-selected regions having N/S and E/W extents of about 125 nautical
 * miles.  The AOI-grid interpolator draws local interpolation windows' support
 *    points from this intermediate grid.  The worldwide grid resides only on
 * disk, and the AOI-GRID algorithm never loads the entire worldwide grid into
 * memory at any one time. The AOI-GRID algorithm allows small-system users to
 * interpolate geoid separations without populating large amounts of high-speed
 * computer memory with geoid separations having no immediate use.  The AOI-GRID
 * algorithm automatically repopulates these intermediate support grids when
 * users' Areas of Interest shift from the 125 nm -by- 125 nm region currently
 * loaded into their computer system's high speed memory.
 *
 ...
 *    interpolate EGM2008 geoid separations using its Area of Interest
 * algorithm. This algorithm may not be fast enough for users needing to quickly
 * compute very large numbers of geoid separations at widely dispersed
 * horizontal locations. The AOI-GRID algorithm is intended for users needing to
 * interpolate many geoid separations in fairly-localized (125 nm -by- 125 nm)
 * areas.
 *
 * REFERENCES
 *
...
const int EGM84_30_MIN_COLS =
    721; /* 360 degrees of longitude at 30 minute spacing */
const int EGM84_30_MIN_ROWS =
    361; /* 180 degrees of latitude  at 30 minute spacing */
const int EGM96_HEADER_ITEMS =
    6; /* min, max lat, min, max long, lat, long spacing*/
const double SCALE_FACTOR_15_MINUTES =
    .25; /* 4 grid cells per degree at 15 minute spacing  */
const double SCALE_FACTOR_10_DEGREES =
    10; /* 1 / 10.0 grid cells per degree at 10 degree spacing */
const double SCALE_FACTOR_30_MINUTES =
    .5; /* 2 grid cells per degree at 30 minute spacing */
const double SCALE_FACTOR_1_DEGREE =
    1; /* 1 grid cell per degree at 1 degree spacing */
const double SCALE_FACTOR_2_DEGREES =
    2; /* 1 / 2 grid cells per degree at 2 degree spacing */
...
const EGM96_Variable_Grid EGM96_Variable_Grid_Table[EGM96_INSET_AREAS] = {
    {74.5, 75.5, 273.5, 280.0},   {66.5, 67.5, 293.5, 295.0},
    {62.5, 64.0, 133.0, 136.5},   {60.5, 61.5, 208.5, 210.0},
    {60.5, 61.0, 219.0, 220.5},   {51.0, 53.0, 172.0, 174.5},
    {52.0, 53.0, 192.5, 194.0},   {51.0, 52.0, 188.5, 191.0},
    {50.0, 52.0, 178.0, 182.5},   {43.0, 46.0, 148.0, 153.5},
    {43.0, 45.0, 84.0, 89.5},     {40.0, 41.0, 70.5, 72.0},
    {36.5, 37.0, 78.5, 79.0},     {36.0, 37.0, 348.0, 349.5},
    {35.0, 36.0, 171.0, 172.5},   {34.0, 35.0, 140.5, 142.0},
    {29.5, 31.0, 78.5, 81.0},     {28.5, 30.0, 81.5, 83.0},
    {26.5, 30.0, 142.0, 143.5},   {26.0, 29.0, 91.5, 96.0},
    {27.5, 29.0, 84.0, 86.5},     {28.0, 29.0, 342.5, 344.0},
    {26.5, 28.0, 88.5, 90.0},     {25.0, 26.0, 189.0, 190.5},
    {23.0, 24.0, 195.0, 196.5},   {21.0, 21.5, 204.0, 204.5},
    {20.0, 21.0, 283.5, 288.0},   {18.5, 20.5, 204.0, 205.5},
    {18.0, 20.0, 291.0, 296.5},   {17.0, 18.0, 298.0, 299.5},
    {15.0, 16.0, 122.0, 123.5},   {12.0, 14.0, 144.5, 147.0},
    {11.0, 12.0, 141.5, 144.0},   {9.5, 11.5, 125.0, 127.5},
    {10.0, 11.0, 286.0, 287.5},   {6.0, 9.5, 287.0, 289.5},
    {5.0, 7.0, 124.0, 128.5},     {-1.0, 1.0, 125.0, 128.5},
    {-3.0, -1.5, 281.0, 282.5},   {-7.0, -5.0, 150.5, 155.0},
    {-8.0, -7.0, 107.0, 108.5},   {-9.0, -7.0, 147.0, 149.5},
    {-11.0, -10.0, 161.5, 163.0}, {-14.5, -13.5, 166.0, 167.5},
    {-18.5, -17.0, 186.5, 188.0}, {-20.5, -20.0, 168.0, 169.5},
    {-23.0, -20.0, 184.5, 187.0}, {-27.0, -24.0, 288.0, 290.5},
    {-53.0, -52.0, 312.0, 313.5}, {-56.0, -55.0, 333.0, 334.5},
    {-61.5, -60.0, 312.5, 317.0}, {-61.5, -60.5, 300.5, 303.0},
    {-73.0, -72.0, 24.5, 26.0}};
...
GeoidLibrary *GeoidLibrary::getInstance() {
  ...
  if (instance == 0) instance = new GeoidLibrary;
...
GeoidLibrary::GeoidLibrary() { loadGeoids(); }
...
void GeoidLibrary::loadGeoids() {
   ...
   * or improperly formatted, an error code of GEOID_INITIALIZE_ERROR is
   * returned, otherwise GEOID_NO_ERROR is returned. This function must be
   * called before any of the other functions in this component.
   *
   * If one or more geoids can be initialized, the function returns
   * successfully.
   */
  ...
  try {
    ...
    if (egm96GeoidList == NULL && egm84GeoidList == NULL &&
        egm84ThirtyMinGeoidList == NULL) {
      throw CoordinateConversionException(
          "Error: GeoidLibrary::LoadGeoids: All geoid height buffer "
...
void GeoidLibrary::convertEllipsoidToEGM96FifteenMinBilinearGeoidHeight(
    ...
    double *geoidHeight) {
  /*
   * The function convertEllipsoidToEGM96FifteenMinBilinearGeoidHeight converts
   * the specified WGS84 ellipsoid height at the specified geodetic coordinates
   * to the equivalent geoid height, using the EGM96 gravity model and the
  ...
  bilinearInterpolate(longitude, latitude, SCALE_FACTOR_15_MINUTES, EGM96_COLS,
                      EGM96_ROWS, egm96GeoidList, &delta_height);
...
void GeoidLibrary::convertEllipsoidToEGM96VariableNaturalSplineHeight(
    ...
    double *geoidHeight) {
  /*
   * The function convertEllipsoidToEGM96VariableNaturalSplineHeight converts
   * the specified WGS84 ellipsoid height at the specified geodetic coordinates
   * to the equivalent geoid height, using the EGM96 gravity model and the
   * natural spline interpolation method..
   *
   *    longitude          : Geodetic longitude in radians          (input)
   *    latitude           : Geodetic latitude in radians           (input)
   *    ellipsoidHeight    : Ellipsoid height, in meters            (input)
  ...
  if (longitude_degrees < 0.0) longitude_degrees += 360.0;
  ...
  while (!found && i < EGM96_INSET_AREAS) {
    if ((latitude_degrees >= EGM96_Variable_Grid_Table[i].min_lat) &&
      ...
      scale_factor =
          SCALE_FACTOR_30_MINUTES;  // use 30 minute by 30 minute grid
      num_cols = 721;
      num_rows = 361;
      found = 1;
    }
  ...
  naturalSplineInterpolate(longitude, latitude, scale_factor, num_cols,
                           num_rows, EGM96_ELEVATIONS - 1, egm96GeoidList,
                           &delta_height);
...
void GeoidLibrary::convertEllipsoidToEGM84TenDegBilinearHeight(
    ...
    double *geoidHeight) {
  /*
   * The function convertEllipsoidToEGM84TenDegBilinearHeight converts the
   * specified WGS84 ellipsoid height at the specified geodetic coordinates to
   * the equivalent geoid height, using the EGM84 gravity model and the bilinear
  ...
  bilinearInterpolate(longitude, latitude, SCALE_FACTOR_10_DEGREES, EGM84_COLS,
                      EGM84_ROWS, egm84GeoidList, &delta_height);
...
void GeoidLibrary::convertEllipsoidToEGM84TenDegNaturalSplineHeight(
    ...
    double *geoidHeight) {
  /*
   * The function convertEllipsoidToEGM84TenDegNaturalSplineHeight converts the
   * specified WGS84 ellipsoid height at the specified geodetic coordinates to
   * the equivalent geoid height, using the EGM84 gravity model and the natural
  ...
  naturalSplineInterpolate(longitude, latitude, SCALE_FACTOR_10_DEGREES,
...
void GeoidLibrary::convertEllipsoidToEGM84ThirtyMinBiLinearHeight(
  ...
  bilinearInterpolateDoubleHeights(longitude, latitude, SCALE_FACTOR_30_MINUTES,
                                   EGM84_30_MIN_COLS, EGM84_30_MIN_ROWS,
...
void GeoidLibrary::convertEllipsoidHeightToEGM2008GeoidHeight(
  ...
  // for two EGM2008 GeoidLibrary ellisoid-height -to- height-above-geoid
  // functions.

  if (this->egm2008Geoid == NULL) {
    throw CoordinateConversionException("Error: EGM2008 geoid buffer is NULL");
  }
  ...
  try {
    ...
    error = this->egm2008Geoid->geoidHeight(WSIZE, latitude, longitude,
                                            geoidSeparation);
...
void GeoidLibrary::convertEGM96FifteenMinBilinearGeoidToEllipsoidHeight(
    double longitude, double latitude, double geoidHeight,
    double *ellipsoidHeight) {
  /*
   * The function convertEGM96FifteenMinBilinearGeoidToEllipsoidHeight converts
   * the specified WGS84 geoid height at the specified geodetic coordinates to
   * the equivalent ellipsoid height, using the EGM96 gravity model and the
   * bilinear interpolation method.
   *
   *    longitude           : Geodetic longitude in radians          (input)
   *    latitude            : Geodetic latitude in radians           (input)
...
void GeoidLibrary::convertEGM96VariableNaturalSplineToEllipsoidHeight(
    ...
    double *ellipsoidHeight) {
  /*
   * The function convertEGM96VariableNaturalSplineToEllipsoidHeight converts
   * the specified WGS84 geoid height at the specified geodetic coordinates to
   * the equivalent ellipsoid height, using the EGM96 gravity model and the
   * natural spline interpolation method.
   *
...
void GeoidLibrary::convertEGM84TenDegBilinearToEllipsoidHeight(
    double longitude, double latitude, double geoidHeight,
    double *ellipsoidHeight) {
  /*
   * The function convertEGM84TenDegBilinearToEllipsoidHeight converts the
...
void GeoidLibrary::convertEGM84TenDegNaturalSplineToEllipsoidHeight(
    ...
    double *ellipsoidHeight) {
  /*
   * The function convertEGM84TenDegNaturalSplineToEllipsoidHeight converts the
   * specified WGS84 geoid height at the specified geodetic coordinates to the
   * equivalent ellipsoid height, using the EGM84 gravity model and the natural
   * spline interpolation method.
   *
   *    longitude           : Geodetic longitude in radians          (input)
   *    latitude            : Geodetic latitude in radians           (input)
...
void GeoidLibrary::convertEGM2008GeoidHeightToEllipsoidHeight(
   ...
   * This function uses the EGM2008 gravity model, plus bicubic spline
   * interpolation.
   *
   *    longitude          : Geodetic longitude in radians          (input)
   *    latitude           : Geodetic latitude in radians           (input)
  ...
  // interpolate a geoid separation from surrounding geoid-separation posts.
  // These two functions have identical software signatures, so there is no need
  // for two EGM2008 GeoidLibrary height-above-geoid -to- ellipsoid_height
  // functions.
...
void GeoidLibrary::initializeEGM96Geoid() {
  /*
   * The function initializeEGM96Geoid reads geoid separation data from the
   * egm96.grd file in the current directory and builds a geoid separation table
   * from it. If the separation file can not be found or accessed, an error code
   * of GEOID_FILE_OPEN_ERROR is returned, If the separation file is incomplete
   * or improperly formatted, an error code of GEOID_INITIALIZE_ERROR is
  ...
  items_discarded = readBinary(egm96GeoidHeaderList, 4, EGM96_HEADER_ITEMS,
                               geoid_height_file);

  /*  Determine if header read properly, or NOT: */

  if (egm96GeoidHeaderList[0] != -90.0 || egm96GeoidHeaderList[1] != 90.0 ||
      egm96GeoidHeaderList[2] != 0.0 || egm96GeoidHeaderList[3] != 360.0 ||
...
void GeoidLibrary::initializeEGM84Geoid() {
  /*
   * The function initializeEGM84Geoid reads geoid separation data from a file
   * in the current directory and builds the geoid separation table from it. If
   * the separation file can not be found or accessed, an error code of
   * GEOID_FILE_OPEN_ERROR is returned, If the separation file is incomplete
   * or improperly formatted, an error code of GEOID_INITIALIZE_ERROR is
   * returned, otherwise GEOID_NO_ERROR is returned.
   */
...
void GeoidLibrary::initializeEGM84ThirtyMinGeoid() {
   ...
   * a file in the current directory and builds the geoid separation table from
   * it. If the separation file can not be found or accessed, an error code of
   * GEOID_FILE_OPEN_ERROR is returned, If the separation file is incomplete
   * or improperly formatted, an error code of GEOID_INITIALIZE_ERROR is
   * returned, otherwise GEOID_NO_ERROR is returned.
   */
...
void GeoidLibrary::bilinearInterpolateDoubleHeights(
    double longitude, double latitude, double scale_factor, int num_cols,
    int num_rows, double *height_buffer, double *delta_height) {
  /*
   * The private function bilinearInterpolateDoubleHeights returns the height of
   * the WGS84 geoid above or below the WGS84 ellipsoid, at the specified
   * geodetic coordinates, using a grid of height adjustments and the bilinear
   * interpolation method.
   *
   *    longitude           : Geodetic longitude in radians          (input)
   *    latitude            : Geodetic latitude in radians           (input)
  ...
  if ((latitude < -PI_OVER_2) ||
      (latitude > PI_OVER_2)) { /* Latitude out of range */
  ...
  post_x = (int)(offset_x);
  if ((post_x + 1) == num_cols) post_x--;
  post_y = (int)(offset_y + 1.0e-11);
  if ((post_y + 1) == num_rows) post_y--;
...
void GeoidLibrary::bilinearInterpolate(double longitude, double latitude,
                                       double scale_factor, int num_cols,
                                       int num_rows, float *height_buffer,
                                       double *delta_height) {
  /*
...
void GeoidLibrary::naturalSplineInterpolate(double longitude, double latitude,
                                            double scale_factor, int num_cols,
                                            int num_rows, int max_index,
                                            float *height_buffer,
                                            double *delta_height) {
  /*
  ...
  if ((temp_offset_x + 1) == num_cols) temp_offset_x--;
  if ((temp_offset_y + 1) == num_rows) temp_offset_y--;
