...
void extractDegrees(char *GEOREFString, double *longitude, double *latitude) {
  ...
  for (i = 0; i < GEOREF_LETTERS; i++) {
    ...
    if ((!isalpha(GEOREFString[i])) || (temp_char == LETTER_I) ||
        (temp_char == LETTER_O)) {
      if ((i == 0) || (i == 2))
...
void extractMinutes(char *GEOREFString, long start, long length, long errorType,
                    double *minutes) {
  /*
   ...
   *
   *    GEOREFString : GEOREF string (input) start        : Start position in
   * the GEOREF string                  (input) length       : length of minutes
   * in the GEOREF string               (input) errorType    : has a value of
   * either GEOREF_STR_LAT_MIN_ERROR  (input) or GEOREF_STR_LON_MIN_ERROR
...
MSP::CCS::GEOREFCoordinates *GEOREF::convertFromGeodetic(
    MSP::CCS::GeodeticCoordinates *geodeticCoordinates, long precision) {
  /*
   *  The function convertFromGeodetic converts Geodetic (latitude and longitude
   * in radians) coordinates to a GEOREF coordinate string.  Precision specifies
   * the number of digits in the GEOREF string for latitude and longitude: 0 for
   * nearest degree 1 for nearest ten minutes 2 for nearest minute 3 for nearest
   * tenth of a minute 4 for nearest hundredth of a minute 5 for nearest
  ...
  if ((latitude < (double)LATITUDE_LOW) || (latitude > (double)LATITUDE_HIGH))
    throw CoordinateConversionException(ErrorMessages::latitude);
  ...
  for (i = 0; i < 4; i++) {
    if (letter_number[i] >= LETTER_I) letter_number[i] += 1;
    if (letter_number[i] >= LETTER_O) letter_number[i] += 1;
  }

  if (letter_number[0] == 26) { /* longitude of 180 degrees */
    letter_number[0] = LETTER_Z;
    letter_number[2] = LETTER_Q;
    long_min = 59.999;
  }
  if (letter_number[1] == 13) { /* latitude of 90 degrees */
...
MSP::CCS::GeodeticCoordinates *GEOREF::convertToGeodetic(
    MSP::CCS::GEOREFCoordinates *georefCoordinates) {
  /*
   *  The function convertToGeodetic converts a GEOREF coordinate string to
   * Geodetic (latitude and longitude in radians) coordinates.
   *
  ...
  extractMinutes(GEOREFString, start, minutes_length, GEOREF_STR_LON_MIN_ERROR,
                 &long_minutes);

  extractMinutes(GEOREFString, (start + minutes_length), minutes_length,
                 GEOREF_STR_LAT_MIN_ERROR, &lat_minutes);
