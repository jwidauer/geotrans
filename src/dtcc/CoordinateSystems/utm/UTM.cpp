 ...
 *	  1-16-16	  AL    MSP_DR30125 added ellipsoid code into
 *constructor and passes ellipsoid code to TransMercator 1-21-16	  KC
 *BAE_MSP00030211, removed the shift from longitude. Shift is applied when
...
UTM::UTM(double ellipsoidSemiMajorAxis, double ellipsoidFlattening,
         char* ellipsoidCode, long override)
    : UTM_Override(0) {
  /*
...
UTMParameters* UTM::getParameters() const {
   ...
   * (output) ellipsoidFlattening    : Flattening of ellipsoid (output) override
   * : UTM override zone, zero indicates no override (output)
   */

  return new UTMParameters(CoordinateType::universalTransverseMercator,
                           UTM_Override);
}

MSP::CCS::UTMCoordinates* UTM::convertFromGeodetic(
    MSP::CCS::GeodeticCoordinates* geodeticCoordinates, int utmZoneOverride) {
  /*
  ...
  if ((latitude < (MIN_LAT - EPSILON)) ||
      (latitude >= (MAX_LAT + EPSILON))) { /* latitude out of range */
  ...
  GeodeticCoordinates tempGeodeticCoordinates(CoordinateType::geodetic,
                                              longitude, latitude);
