...
MillerCylindrical::MillerCylindrical(double ellipsoidSemiMajorAxis,
   ...
   * Miller Cylindrical projcetion parameters as inputs, and sets the
...
MapProjection3Parameters* MillerCylindrical::getParameters() const {
  ...
  return new MapProjection3Parameters(CoordinateType::millerCylindrical,
                                      Mill_Origin_Long, Mill_False_Easting,
                                      Mill_False_Northing);
}

MSP::CCS::MapProjectionCoordinates* MillerCylindrical::convertFromGeodetic(
    MSP::CCS::GeodeticCoordinates* geodeticCoordinates) {
  /*
   * The function convertFromGeodetic converts geodetic (latitude and
   * longitude) coordinates to Miller Cylindrical projection (easting and
   * northing) coordinates, according to the current ellipsoid and Miller
   * Cylindrical projection parameters.  If any errors occur, an exception is
  ...
  double northing =
      (Ra / 1.6) * log((1.0 + slat) / (1.0 - slat)) + Mill_False_Northing;
...
MSP::CCS::GeodeticCoordinates* MillerCylindrical::convertToGeodetic(
   ...
   * coordinates, according to the current ellipsoid and Miller Cylindrical
  ...
  if ((easting < (Mill_False_Easting + Mill_Min_Easting)) ||
      (easting >
       (Mill_False_Easting + Mill_Max_Easting))) { /* Easting out of range  */
    throw CoordinateConversionException(ErrorMessages::easting);
  }
  if ((northing < (Mill_False_Northing - Mill_Delta_Northing)) ||
      (northing > (Mill_False_Northing +
                   Mill_Delta_Northing))) { /* Northing out of range */
