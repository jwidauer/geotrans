...
MapProjection4Parameters* Orthographic::getParameters() const {
  ...
  return new MapProjection4Parameters(CoordinateType::orthographic,
                                      Orth_Origin_Long, Orth_Origin_Lat,
                                      Orth_False_Easting, Orth_False_Northing);
}

MSP::CCS::MapProjectionCoordinates* Orthographic::convertFromGeodetic(
  ...
  if (cos_c < 0.0) { /* Point is out of view.  Will return longitude out of
                  range message since no point out of view is implemented.  */
    throw CoordinateConversionException(ErrorMessages::longitude);
  }
  ...
  double northing =
      Ra * (Cos_Orth_Origin_Lat * slat - Sin_Orth_Origin_Lat * clat_cdlam) +
      Orth_False_Northing;

  return new MapProjectionCoordinates(CoordinateType::orthographic, easting,
                                      northing);
}

MSP::CCS::GeodeticCoordinates* Orthographic::convertToGeodetic(
  ...
  if ((easting > (Orth_False_Easting + Ra)) ||
      (easting < (Orth_False_Easting - Ra))) { /* Easting out of range */
    throw CoordinateConversionException(ErrorMessages::easting);
  }
  if ((northing > (Orth_False_Northing + Ra)) ||
      (northing < (Orth_False_Northing - Ra))) { /* Northing out of range */
  ...
  if ((temp > (Orth_False_Easting + Ra)) ||
      ...
      (temp <
       (Orth_False_Northing - Ra))) { /* Point is outside of projection area */
  ...
  if (rho == 0.0) {
    ...
    cos_cc = cos(cc);
    sin_cc = sin(cc);
    latitude = asin(cos_cc * Sin_Orth_Origin_Lat +
                    (dy * sin_cc * Cos_Orth_Origin_Lat / rho));
    ...
    if (Orth_Origin_Lat == MAX_LAT)
      ...
      longitude = Orth_Origin_Long +
                  atan2(dx * sin_cc, (rho * Cos_Orth_Origin_Lat * cos_cc -
                                      dy * Sin_Orth_Origin_Lat * sin_cc));
