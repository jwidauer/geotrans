...
Gnomonic::Gnomonic(double ellipsoidSemiMajorAxis, double ellipsoidFlattening,
...
MapProjection4Parameters* Gnomonic::getParameters() const {
  ...
  return new MapProjection4Parameters(CoordinateType::gnomonic,
                                      Gnom_Origin_Long, Gnom_Origin_Lat,
...
MSP::CCS::MapProjectionCoordinates* Gnomonic::convertFromGeodetic(
  ...
  if (fabs(abs_Gnom_Origin_Lat - PI_OVER_2) < 1.0e-10) {
    ...
    northing = Ra_kprime * (Cos_Gnom_Origin_Lat * slat -
                            Sin_Gnom_Origin_Lat * clat * cos_dlam) +
               Gnom_False_Northing;
  }

  return new MapProjectionCoordinates(CoordinateType::gnomonic, easting,
                                      northing);
}
...
MSP::CCS::GeodeticCoordinates* Gnomonic::convertToGeodetic(
  ...
  if ((easting < (Gnom_False_Easting - Gnom_Delta_Easting)) ||
      (easting >
       (Gnom_False_Easting + Gnom_Delta_Easting))) { /* Easting out of range  */
    throw CoordinateConversionException(ErrorMessages::easting);
  }
  if ((northing < (Gnom_False_Northing - Gnom_Delta_Northing)) ||
      (northing > (Gnom_False_Northing +
                   Gnom_Delta_Northing))) { /* Northing out of range */
  ...
  if (fabs(rho) <= 1.0e-10) {
    ...
    if (fabs(abs_Gnom_Origin_Lat - PI_OVER_2) < 1.0e-10) {
      ...
      longitude = Gnom_Origin_Long +
                  atan2((dx * sin_c), (rho * Cos_Gnom_Origin_Lat * cos_c -
                                       dy_sinc * Sin_Gnom_Origin_Lat));
  }
