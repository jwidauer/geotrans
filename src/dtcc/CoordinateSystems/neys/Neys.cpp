 ...
 *    LambertConformalConic2.h  - Is used to convert lambert conformal conic
 * coordinates Neys.h     - Is for prototype error checking
...
////const double MAX_LAT = (89.99972222222222 * PI / 180.0); /* 89 59 59.0
///degrees     */
const double MAX_LAT =
    (89.999444444444444 * PI_OVER_180); /* 89 59 58.0 degrees     */
...
Neys::Neys(double ellipsoidSemiMajorAxis, double ellipsoidFlattening,
  ...
  if ((fabs(standardParallel - SEVENTY_ONE) > epsilon) &&
      (fabs(standardParallel - SEVENTY_FOUR) >
       epsilon)) { /* First Standard Parallel invalid */
  ...
  lambertConformalConic2 = new LambertConformalConic(
      semiMajorAxis, flattening, Neys_Origin_Long, Neys_Origin_Lat,
      Neys_Std_Parallel_1, Neys_Std_Parallel_2, Neys_False_Easting,
      Neys_False_Northing);
}
...
NeysParameters* Neys::getParameters() const {
  ...
  return new NeysParameters(CoordinateType::neys, Neys_Origin_Long,
                            Neys_Origin_Lat, Neys_Std_Parallel_1,
                            Neys_False_Easting, Neys_False_Northing);
}

MSP::CCS::MapProjectionCoordinates* Neys::convertFromGeodetic(
...
MSP::CCS::GeodeticCoordinates* Neys::convertToGeodetic(
  ...
  if ((easting < (Neys_False_Easting - Neys_Delta_Easting)) ||
      (easting >
       (Neys_False_Easting + Neys_Delta_Easting))) { /* Easting out of range  */
    throw CoordinateConversionException(ErrorMessages::easting);
  }
  if ((northing < (Neys_False_Northing - Neys_Delta_Northing)) ||
      (northing > (Neys_False_Northing +
                   Neys_Delta_Northing))) { /* Northing out of range */
