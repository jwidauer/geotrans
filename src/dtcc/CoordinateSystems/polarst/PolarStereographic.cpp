...
PolarStereographic::PolarStereographic(double ellipsoidSemiMajorAxis,
   ...
   * as inputs, and sets the corresponding state variables.  If any errors
   ...
   *  falseEasting       : Easting (X) at center of projection, in meters
   * (input) falseNorthing      : Northing (Y) at center of projection, in
   * meters (input)
   */
  ...
  if ((standardParallel < -PI_OVER_2) ||
      (standardParallel > PI_OVER_2)) { /* Origin Latitude out of range */
  ...
  one_MINUS_es = 1.0 - es;
  Polar_k90 =
      sqrt(pow(one_PLUS_es, one_PLUS_es) * pow(one_MINUS_es, one_MINUS_es));
  ...
  Polar_Scale_Factor =
      ((1 + slat) / 2) *
      (Polar_k90 / sqrt(pow(one_PLUS_es_sinolat, one_PLUS_es) *
                        pow(one_MINUS_es_sinolat, one_MINUS_es)));

  /* Calculate Radius */
  GeodeticCoordinates tempGeodeticCoordinates =
      GeodeticCoordinates(CoordinateType::geodetic, centralMeridian, 0);
  // MapProjectionCoordinates* tempCoordinates = convertFromGeodetic(
  // (GeodeticCoordinates&) GeodeticCoordinates( CoordinateType::geodetic,
  // centralMeridian, 0 ) );
  MapProjectionCoordinates* tempCoordinates =
      convertFromGeodetic(&tempGeodeticCoordinates);
  Polar_Delta_Northing = tempCoordinates->northing();
  delete tempCoordinates;

  if (Polar_False_Northing) Polar_Delta_Northing -= Polar_False_Northing;
  if (Polar_Delta_Northing < 0) Polar_Delta_Northing = -Polar_Delta_Northing;
...
PolarStereographic::PolarStereographic(double ellipsoidSemiMajorAxis,
                                       double ellipsoidFlattening,
                                       double centralMeridian,
                                       double scaleFactor, char hemisphere,
                                       double falseEasting,
                                       double falseNorthing)
    : CoordinateSystem(),
  ...
  while (fabs(sk_PLUS_1 - sk) > tolerance && count) {
    ...
    sk_PLUS_1 = ((2 * Polar_Scale_Factor *
                  sqrt(pow(one_PLUS_es_sk, one_PLUS_es) *
                       pow(one_MINUS_es_sk, one_MINUS_es))) /
                 Polar_k90) -
                1;
    count--;
  }
  ...
  if (hemisphere == 'S') standardParallel *= -1.0;
...
PolarStereographic::getStandardParallelParameters() const {
  /*
   ...
   * (output) centralMeridian                 : Longitude down from pole, in
   * radians            (output) standardParallel                : Latitude of
   * true scale, in radians              (output) falseEasting : Easting (X) at
   * center of projection, in meters  (output) falseNorthing                   :
   * Northing (Y) at center of projection, in meters (output)
   */

  return new PolarStereographicStandardParallelParameters(
      CoordinateType::polarStereographicStandardParallel,
      Polar_Central_Meridian, Polar_Standard_Parallel, Polar_False_Easting,
      Polar_False_Northing);
}

PolarStereographicScaleFactorParameters*
PolarStereographic::getScaleFactorParameters() const {
  /*
   ...
   * (output) ellipsoidFlattening             : Flattening of ellipsoid
   * (output) centralMeridian                 : Longitude down from pole, in
   * radians            (output) scaleFactor                     : Scale factor
   * (output) falseEasting                    : Easting (X) at center of
   * projection, in meters  (output) falseNorthing                   : Northing
   * (Y) at center of projection, in meters (output)
   */

  if (Southern_Hemisphere == 0)
    return new PolarStereographicScaleFactorParameters(
        CoordinateType::polarStereographicScaleFactor, Polar_Central_Meridian,
        Polar_Scale_Factor, 'N', Polar_False_Easting, Polar_False_Northing);
  else
    return new PolarStereographicScaleFactorParameters(
        CoordinateType::polarStereographicScaleFactor, Polar_Central_Meridian,
        Polar_Scale_Factor, 'S', Polar_False_Easting, Polar_False_Northing);
}

MSP::CCS::MapProjectionCoordinates* PolarStereographic::convertFromGeodetic(
...
MSP::CCS::GeodeticCoordinates* PolarStereographic::convertToGeodetic(
  ...
  if (easting > max_easting ||
      easting < min_easting) { /* easting out of range */
    throw CoordinateConversionException(ErrorMessages::easting);
  }
  if (northing > max_northing ||
      northing < min_northing) { /* northing out of range */
  ...
  delta_radius = sqrt(Polar_Delta_Easting * Polar_Delta_Easting +
