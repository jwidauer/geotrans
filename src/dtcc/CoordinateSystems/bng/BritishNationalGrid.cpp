...
const char* BNG100GRID =
    "VWXYZQRSTULMNOPFGHJKABCDE"; /* 100,000 unit square identifications */

/* BNG projection Parameters */
const double BNG_Origin_Lat =
    (49.0 * PI / 180.0);  // Latitude of origin, in radians
const double BNG_Origin_Long =
    (-2.0 * PI / 180.0);                      // Longitude of origin, in radians
...
void findIndex(char letter, const char* letterArray, long* index) {
  ...
  if (not_Found) throw CoordinateConversionException(ErrorMessages::bngString);
}
...
void breakBNGString(char* BNGString, char letters[3], double* easting,
  ...
  while (BNGString[i] == ' ') i++; /* skip any leading blanks */
  j = i;
  while (isalpha(BNGString[i])) i++;
  num_letters = i - j;
  if (num_letters == 2) {
    /* get letters */
...
BritishNationalGrid::BritishNationalGrid(char* ellipsoidCode)
  ...
  if (strcmp(ellipsoidCode, Airy) != 0) { /* Ellipsoid must be Airy */
...
BritishNationalGrid& BritishNationalGrid::operator=(
    const BritishNationalGrid& bng) {
  if (this != &bng) {
...
EllipsoidParameters* BritishNationalGrid::getParameters() const {
  ...
  return new EllipsoidParameters(semiMajorAxis, flattening,
                                 (char*)BNG_Ellipsoid_Code);
}

MSP::CCS::BNGCoordinates* BritishNationalGrid::convertFromGeodetic(
  ...
  if ((longitude < MIN_LON) ||
      (longitude > MAX_LON)) { /* Longitude out of range */
  ...
  try {
    bngCoordinates =
        convertFromTransverseMercator(transverseMercatorCoordinates, precision);
    delete transverseMercatorCoordinates;
...
MSP::CCS::GeodeticCoordinates* BritishNationalGrid::convertToGeodetic(
  ...
  breakBNGString(BNGString, BNG_Letters, &BNG_Easting, &BNG_Northing,
                 &in_Precision);

  MapProjectionCoordinates* transverseMercatorCoordinates =
      convertToTransverseMercator(bngCoordinates);
  TMEasting = transverseMercatorCoordinates->easting();
  TMNorthing = transverseMercatorCoordinates->northing();
...
MSP::CCS::BNGCoordinates* BritishNationalGrid::convertFromTransverseMercator(
  ...
  if ((easting < BNG_Min_Easting) ||
      (easting > BNG_Max_Easting)) { /* Easting out of range  */
    throw CoordinateConversionException(ErrorMessages::easting);
  }
  if ((northing < BNG_Min_Northing) ||
      (northing > BNG_Max_Northing)) { /* Northing out of range */
...
BritishNationalGrid::convertToTransverseMercator(
    MSP::CCS::BNGCoordinates* bngCoordinates) {
  /*
   ...
   * exception is thrown with a description of the error. is returned.
   *
   *    BNGString  : British National Grid coordinate string (input)
