 ...
 *    (1 or 2 Standard Parallel) projection coordinates (easting and northing in
 *meters) defined by one standard parallel and specified scale true along that
 *parallel, or two standard parallels. When both standard parallel parameters
 ...
 *                                      range (-89 59 59.0 to 89 59 59.0
 *degrees) or within one second of 0 degrees LAMBERT_CENT_MER_ERROR     :
 *Central meridian outside of valid range
 *                                      (-180 to 360 degrees)
 *		   LAMBERT_SCALE_FACTOR_ERROR : Scale factor greater than minimum
 *scale factor (1.0e-9) LAMBERT_A_ERROR            : Semi-major axis less than
 *or equal to zero LAMBERT_INV_F_ERROR        : Inverse flattening outside of
...
const double MAX_LAT =
    ((PI * 89.99972222222222) / 180.0); /* 89 59 59.0 degrees in radians */
...
LambertConformalConic::LambertConformalConic(
   ...
   * If any errors occur, an exception is thrown with a description of the
   * error.
   *
   ...
   * Latitude of origin, in radians            (input) falseEasting           :
   * False easting, in meters                  (input) falseNorthing          :
   * False northing, in meters                 (input) scaleFactor            :
   * Projection scale factor                   (input)
   *
   */
  ...
  if (ellipsoidSemiMajorAxis <=
      0.0) { /* Semi-major axis must be greater than zero */
    throw CoordinateConversionException(ErrorMessages::semiMajorAxis);
  }
  if ((inv_f < 250) ||
      (inv_f > 350)) { /* Inverse flattening must be between 250 and 350 */
    throw CoordinateConversionException(ErrorMessages::ellipsoidFlattening);
  }
  if ((centralMeridian < -PI) ||
      (centralMeridian > TWO_PI)) { /* Origin Longitude out of range */
  ...
  double phi = 0.0;
  double tempPhi = 1.0;
  Lambert_2_Std_Parallel_1 =
      calculateLambert2StandardParallel(es2, phi, tempPhi, c);

  phi = 89.0 * PI_OVER_180;
  tempPhi = 0.0;
  Lambert_2_Std_Parallel_2 =
      calculateLambert2StandardParallel(es2, phi, tempPhi, c);
...
LambertConformalConic::LambertConformalConic(
    double ellipsoidSemiMajorAxis, double ellipsoidFlattening,
    double centralMeridian, double originLatitude, double standardParallel1,
    double standardParallel2, double falseEasting, double falseNorthing)
    : CoordinateSystem(),
  ...
  /*
   * The constructor receives the ellipsoid parameters and
   * Lambert Conformal Conic (2 Standard Parallel) projection parameters as
   * inputs, and sets the corresponding state variables.  If any errors occur,
   ...
   * (input) centralMeridian          : Longitude of origin, in radians (input)
   ...
   *   falseEasting             : False easting, in meters (input) falseNorthing
  ...
  if ((originLatitude < -MAX_LAT) ||
      (originLatitude > MAX_LAT)) { /* Origin Latitude out of range */
    throw CoordinateConversionException(ErrorMessages::originLatitude);
  }
  if ((standardParallel1 < -MAX_LAT) ||
      (standardParallel1 >
       MAX_LAT)) { /* First Standard Parallel out of range */
    throw CoordinateConversionException(ErrorMessages::standardParallel1);
  }
  if ((standardParallel2 < -MAX_LAT) ||
      (standardParallel2 >
       MAX_LAT)) { /* Second Standard Parallel out of range */
    throw CoordinateConversionException(ErrorMessages::standardParallel2);
  }
  if ((standardParallel1 == 0) &&
      (standardParallel2 ==
       0)) { /* First & Second Standard Parallels are both 0 */
    throw CoordinateConversionException(ErrorMessages::standardParallel1_2);
  }
  if (standardParallel1 ==
      -standardParallel2) { /* Parallels are the negation of each other */
    throw CoordinateConversionException(
        ErrorMessages::standardParallelHemisphere);
  }
  if ((centralMeridian < -PI) ||
      (centralMeridian > TWO_PI)) { /* Central meridian out of range */
    throw CoordinateConversionException(ErrorMessages::centralMeridian);
  }
  ...
  if (fabs(Lambert_2_Std_Parallel_1 - Lambert_2_Std_Parallel_2) > 1.0e-10) {
    ...
    Lambert_false_northing = (const_value * pow(t_olat, n)) -
                             (const_value * pow(t0, n)) + falseNorthing;
  } else {
...
LambertConformalConic::get1StandardParallelParameters() const {
  /*
   * The function get1StandardParallelParameters returns the current ellipsoid
   * parameters and Lambert Conformal Conic (1 Standard Parallel) projection
   * parameters.
   *
   *   ellipsoidSemiMajorAxis  : Semi-major axis of ellipsoid, in meters
   ...
   * : False northing, in meters               (output) scaleFactor : Projection
   * scale factor                 (output)
   */

  return new MapProjection5Parameters(
      CoordinateType::lambertConformalConic1Parallel, Lambert_Origin_Long,
      Lambert_Origin_Latitude, Lambert_Scale_Factor, Lambert_False_Easting,
      Lambert_False_Northing);
}

MapProjection6Parameters*
LambertConformalConic::get2StandardParallelParameters() const {
  /*
   * The function get2StandardParallelParameters returns the current ellipsoid
   * parameters and Lambert Conformal Conic (2 Standard Parallel) projection
   * parameters.
   *
   *   ellipsoidSemiMajorAxis  : Semi-major axis of ellipsoid, in meters
   * (output) ellipsoidFlattening     : Flattening of ellipsoid (output)
  ...
  return new MapProjection6Parameters(
      CoordinateType::lambertConformalConic2Parallels, Lambert_Origin_Long,
      Lambert_2_Origin_Lat, Lambert_2_Std_Parallel_1, Lambert_2_Std_Parallel_2,
...
MSP::CCS::MapProjectionCoordinates* LambertConformalConic::convertFromGeodetic(
   ...
   * longitude) coordinates to Lambert Conformal Conic (1 or 2 Standard
   * Parallel) projection (easting and northing) coordinates, according to the
   * current ellipsoid and Lambert Conformal Conic (1 or 2 Standard Parallel)
  ...
  if (fabs(fabs(latitude) - PI_OVER_2) > 1.0e-10) {
    ...
    if ((latitude * Lambert_1_n) <= 0) { /* Point can not be projected */
      throw CoordinateConversionException(ErrorMessages::latitude);
    }
    rho = 0.0;
  }
  ...
  double northing =
      Lambert_1_rho_olat - rho * cos(theta) + Lambert_False_Northing;

  return new MapProjectionCoordinates(coordinateType, easting, northing);
}

MSP::CCS::GeodeticCoordinates* LambertConformalConic::convertToGeodetic(
    MSP::CCS::MapProjectionCoordinates* mapProjectionCoordinates) {
  /*
   * The function convertToGeodetic converts Lambert Conformal
   * Conic (1 or 2 Standard Parallel) projection (easting and northing)
   * coordinates to Geodetic (latitude and longitude) coordinates, according to
   * the current ellipsoid and Lambert Conformal Conic (1 or 2 Standard
   * Parallel) projection parameters.  If any errors occur, an exception is
  ...
  if ((easting < (Lambert_False_Easting - Lambert_Delta_Easting)) ||
      (easting > (Lambert_False_Easting +
                  Lambert_Delta_Easting))) { /* Easting out of range  */
    throw CoordinateConversionException(ErrorMessages::easting);
  }
  if ((northing < (Lambert_False_Northing - Lambert_Delta_Northing)) ||
      (northing > (Lambert_False_Northing +
                   Lambert_Delta_Northing))) { /* Northing out of range */
  ...
  if (rho != 0.0) {
    ...
    while (fabs(PHI - tempPHI) > tolerance && count) {
      ...
      PHI = PI_OVER_2 -
            2.0 * atan(t * pow((1.0 - es_sin) / (1.0 + es_sin), es_OVER_2));
      count--;
    }

    if (!count) throw CoordinateConversionException(ErrorMessages::northing);
...
double LambertConformalConic::calculateLambert2StandardParallel(double es2,
                                                                double phi,
                                                                double tempPhi,
                                                                double c) {
  /*
  ...
  while (fabs(phi - tempPhi) > tolerance && count > 0) {
    ...
    double fprPhi = ((Lambert_1_n - sinPhi) * (1 - es2)) /
                    (pow(wPhi, 3) * pow(tPhi, Lambert_1_n));
...
double LambertConformalConic::lambertT(double lat, double essin) {
  return tan(PI_OVER_4 - lat / 2) /
         pow((1.0 - essin) / (1.0 + essin), es_OVER_2);
}

double LambertConformalConic::esSin(double sinlat) { return es * sinlat; }
