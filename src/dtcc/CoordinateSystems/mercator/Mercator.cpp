 ...
 *                                           depending on ellipsoid parameters
 *                                           and Origin_Latitude)
 *          MERC_LAT_OF_TRUE_SCALE_ERROR   : Latitude of true scale outside of
 *valid range
 *                                           (-89.5 to 89.5 degrees)
 *          MERC_CENT_MER_ERROR            : Central meridian outside of valid
 *range
 *                                           (-180 to 360 degrees)
 *          MERC_A_ERROR                   : Semi-major axis less than or equal
 *to zero MERC_INV_F_ERROR               : Inverse flattening outside of valid
 *range (250 to 350)
 *
 * REUSE NOTES
 *
...
Mercator::Mercator(double ellipsoidSemiMajorAxis, double ellipsoidFlattening,
                   double centralMeridian, double standardParallel,
   ...
   * Mercator (Standard Parallel) projection parameters as inputs, and sets the
   * corresponding state variables.  It calculates and returns the scale factor.
   ...
   * (input) the projection standardParallel        : Latitude in radians at
  ...
  if ((standardParallel < -MAX_LAT) ||
      (standardParallel > MAX_LAT)) { /* latitude of true scale out of range */
  ...
  es4 = es3 * Merc_es;
  Merc_ab =
      Merc_es / 2.e0 + 5.e0 * es2 / 24.e0 + es3 / 12.e0 + 13.e0 * es4 / 360.e0;
  Merc_bb = 7.e0 * es2 / 48.e0 + 29.e0 * es3 / 240.e0 + 811.e0 * es4 / 11520.e0;
  ...
  *scaleFactor = Merc_Scale_Factor;
  ...
  GeodeticCoordinates gcTemp(CoordinateType::geodetic, (Merc_Cent_Mer + PI),
                             MAX_LAT);
  ...
  if (Merc_False_Easting) Merc_Delta_Easting -= Merc_False_Easting;
  if (Merc_Delta_Easting < 0) Merc_Delta_Easting = -Merc_Delta_Easting;
  Merc_Delta_Easting *= 1.01;

  if (Merc_False_Northing) Merc_Delta_Northing -= Merc_False_Northing;
...
Mercator::Mercator(double ellipsoidSemiMajorAxis, double ellipsoidFlattening,
  ...
  /*
   * The constructor receives the ellipsoid parameters and
   * Mercator (Scale Factor) projection parameters as inputs, and sets the
   * corresponding state variables.  It receives the scale factor as input.  If
  ...
  Merc_False_Northing = falseNorthing;
  Merc_False_Easting = falseEasting;
  Merc_Scale_Factor = scaleFactor;  /// cos(Merc_Standard_Parallel) / sqrt(1.e0
                                    /// - Merc_es * sin_olat * sin_olat );

  Merc_es = 2 * flattening - flattening * flattening;
  Merc_e = sqrt(Merc_es);
  //  sin_olat = sin(Merc_Standard_Parallel);
  //  Merc_Scale_Factor = cos(Merc_Standard_Parallel) / sqrt(1.e0 - Merc_es *
  //  sin_olat * sin_olat ); Merc_Scale_Factor = 1.0 / ( sqrt(1.e0 - Merc_es *
  //  sin_olat * sin_olat)
  //                              / cos(Merc_Standard_Parallel) );
  es2 = Merc_es * Merc_es;
  es3 = es2 * Merc_es;
  ...
  Merc_Standard_Parallel =
      asin(sqrt((1 - Merc_Scale_Factor2) / (1 - Merc_Scale_Factor2 * Merc_es)));
...
MercatorStandardParallelParameters* Mercator::getStandardParallelParameters()
    const {
  /*
   ...
   * (output) the projection standardParallel     : Latitude in radians at which
   * the          (output) point scale factor is 1.0 falseEasting            : A
   * coordinate value in meters assigned to the central meridian of the
   * projection.       (output) falseNorthing           : A coordinate value in
   * meters assigned to the origin latitude of the projection         (output)
   */

  return new MercatorStandardParallelParameters(
      CoordinateType::mercatorStandardParallel, Merc_Cent_Mer,
      Merc_Standard_Parallel, Merc_Scale_Factor, Merc_False_Easting,
      Merc_False_Northing);
}
...
MercatorScaleFactorParameters* Mercator::getScaleFactorParameters() const {
  ...
  return new MercatorScaleFactorParameters(
      CoordinateType::mercatorScaleFactor, Merc_Cent_Mer, Merc_Scale_Factor,
      Merc_False_Easting, Merc_False_Northing);
}

MSP::CCS::MapProjectionCoordinates* Mercator::convertFromGeodetic(
  ...
  if ((latitude < -MAX_LAT) ||
      (latitude > MAX_LAT)) { /* Latitude out of range */
  ...
  pow_temp = pow(((1.e0 - e_x_sinlat) / (1.e0 + e_x_sinlat)), (Merc_e / 2.e0));
  ctanz2 = tan_temp * pow_temp;
  double northing =
      Merc_Scale_Factor * semiMajorAxis * log(ctanz2) + Merc_False_Northing;
  Delta_Long = longitude - Merc_Cent_Mer;
  if (Delta_Long > PI) Delta_Long -= TWO_PI;
  if (Delta_Long < -PI) Delta_Long += TWO_PI;
  double easting =
      Merc_Scale_Factor * semiMajorAxis * Delta_Long + Merc_False_Easting;

  return new MapProjectionCoordinates(coordinateType, easting, northing);
}

MSP::CCS::GeodeticCoordinates* Mercator::convertToGeodetic(
  ...
  if ((easting < (Merc_False_Easting - Merc_Delta_Easting)) ||
      (easting >
       (Merc_False_Easting + Merc_Delta_Easting))) { /* Easting out of range */
    throw CoordinateConversionException(ErrorMessages::easting);
  }
  if ((northing < (Merc_False_Northing - Merc_Delta_Northing)) ||
      (northing > (Merc_False_Northing +
                   Merc_Delta_Northing))) { /* Northing out of range */
  ...
  double latitude = xphi + Merc_ab * sin(2.e0 * xphi) +
                    Merc_bb * sin(4.e0 * xphi) + Merc_cb * sin(6.e0 * xphi) +
                    Merc_db * sin(8.e0 * xphi);
