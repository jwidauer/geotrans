...
#define LOGW(...) \
  ((void)__android_log_print(ANDROID_LOG_WARN, "GtApp", __VA_ARGS__))
#endif
 ...
 *    EllipsoidLibraryImplementation.h  - prototype error checking and error
 * codes Ellipsoid.h  - used to store individual ellipsoid information threads.h
 * - used for thread safety CoordinateConversionException.h - Exception handler
...
const int ELLIPSOID_CODE_LENGTH =
    3; /* Length of ellipsoid code (including null) */
...
EllipsoidLibraryImplementation *EllipsoidLibraryImplementation::getInstance() {
  ...
  if (instance == 0) instance = new EllipsoidLibraryImplementation;
...
EllipsoidLibraryImplementation::EllipsoidLibraryImplementation(
    const EllipsoidLibraryImplementation &el) {
...
EllipsoidLibraryImplementation &EllipsoidLibraryImplementation::operator=(
    const EllipsoidLibraryImplementation &el) {
...
void EllipsoidLibraryImplementation::defineEllipsoid(const char *code,
...
#ifdef NDK_BUILD
  __android_log_print(ANDROID_LOG_VERBOSE, "GtApp", "numEllipsoid %d ",
                      numEllipsoids);
#endif
  ...
  ellipsoidList.push_back(new Ellipsoid(index, ellipsoid_code, (char *)name,
                                        ...
                                        flattening, eccentricitySquared, true));
  ...
  if ((fp = fopen(FileName, "w")) == NULL) { /* fatal error */
  ...
  while (index < numEllipsoids) {
    if (ellipsoidList[index]->userDefined()) {
      fprintf(fp, "*%-28s  %-2s %11.9f %12.9f %13.13f \n",
              ellipsoidList[index]->name(), ellipsoidList[index]->code(),
...
#ifdef NDK_BUILD
  __android_log_print(ANDROID_LOG_VERBOSE, "GtApp", "ellipsoid code %s %d ",
                      code, numEllipsoids);
#endif

  while ((i < numEllipsoids) && strcmp(temp_code, ellipsoidList[i]->code())) {
    i++;
  }
...
void EllipsoidLibraryImplementation::setDatumLibraryImplementation(
...
void EllipsoidLibraryImplementation::loadEllipsoids() {
  ...
  while (!feof(fp)) {
    if (fgets(buffer, ELLIPSOID_BUF, fp)) {
      ...
      sscanf(buffer, "%30c %s %lf %lf %lf", name, code, &semiMajorAxis,
             &semiMinorAxis, &recpF);

      bool userDefined = false; /* Identifies a user defined ellipsoid */
      if (name[0] == '*') {
        userDefined = true;
        for (int i = 0; i < ELLIPSOID_NAME_LENGTH; i++) name[i] = name[i + 1];
      }
      ...
      __android_log_print(ANDROID_LOG_VERBOSE, "GtApp", "major %f",
                          semiMajorAxis);
      __android_log_print(ANDROID_LOG_VERBOSE, "GtApp", "minor %f",
                          semiMinorAxis);
      __android_log_print(ANDROID_LOG_VERBOSE, "GtApp", "recpF %f", recpF);
#endif

      ellipsoidList.push_back(new Ellipsoid(index, code, name, semiMajorAxis,
                                            semiMinorAxis, flattening,
                                            eccentricitySquared, userDefined));
