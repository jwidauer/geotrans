...
double oneMinusSqr(double x) { return 1.0 - x * x; }
...
AlbersEqualAreaConic::AlbersEqualAreaConic(
   ...
   * scale factor is 1.0 standardParallel1       : First standard parallel
   * (input) standardParallel2       : Second standard parallel (input)
  ...
  if ((standardParallel1 < -PI_OVER_2) ||
      (standardParallel1 >
       PI_OVER_2)) { /* First Standard Parallel out of range */
    throw CoordinateConversionException(ErrorMessages::standardParallel1);
  }
  if ((standardParallel2 < -PI_OVER_2) ||
      (standardParallel2 >
       PI_OVER_2)) { /* Second Standard Parallel out of range */
    throw CoordinateConversionException(ErrorMessages::standardParallel2);
  }
  if ((standardParallel1 == 0.0) &&
      (standardParallel2 ==
       0.0)) { /* First & Second Standard Parallels equal 0 */
    throw CoordinateConversionException(ErrorMessages::standardParallel1_2);
  }
  if (standardParallel1 ==
      -standardParallel2) { /* Parallels are opposite latitudes */
...
MapProjection6Parameters* AlbersEqualAreaConic::getParameters() const {
   ...
   *  originLatitude          : Latitude in radians at which the (output) point
   * scale factor is 1.0 standardParallel1       : First standard parallel
   * (output) standardParallel2       : Second standard parallel (output)
   *  falseEasting            : A coordinate value in meters assigned to the
  ...
  return new MapProjection6Parameters(
      CoordinateType::albersEqualAreaConic, Albers_Origin_Long,
      Albers_Origin_Lat, Albers_Std_Parallel_1, Albers_Std_Parallel_2,
      Albers_False_Easting, Albers_False_Northing);
}
...
MSP::CCS::GeodeticCoordinates* AlbersEqualAreaConic::convertToGeodetic(
  ...
  if ((easting < (Albers_False_Easting - Albers_Delta_Easting)) ||
      (easting > Albers_False_Easting +
                     Albers_Delta_Easting)) { /* Easting out of range  */
    throw CoordinateConversionException(ErrorMessages::easting);
  }
  if ((northing < (Albers_False_Northing - Albers_Delta_Northing)) ||
      (northing > Albers_False_Northing +
                      Albers_Delta_Northing)) { /* Northing out of range */
    ...
    if (q_OVER_2 > 1.0)
      ...
      if (es < 1.0e-10)
        ...
        while ((fabs(Delta_PHI) > tolerance) && count) {
          ...
          one_MINUS_SQRes_sin = oneMinusSqr(es_sin);
          Delta_PHI = (one_MINUS_SQRes_sin * one_MINUS_SQRes_sin) /
                      (2.0 * cos(PHI)) *
...
double AlbersEqualAreaConic::esSine(double sinlat) { return es * sinlat; }

double AlbersEqualAreaConic::albersQ(double slat, double oneminussqressin,
                                     double essin) {
