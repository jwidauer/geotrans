...
Eckert6::Eckert6(double ellipsoidSemiMajorAxis, double ellipsoidFlattening,
...
MapProjection3Parameters* Eckert6::getParameters() const {
  ...
  return new MapProjection3Parameters(CoordinateType::eckert6, Eck6_Origin_Long,
                                      Eck6_False_Easting, Eck6_False_Northing);
}
...
MSP::CCS::MapProjectionCoordinates* Eckert6::convertFromGeodetic(
  ...
  while (fabs(delta_theta) > dt_tolerance && count) {
    delta_theta =
        -(theta + sin(theta) - one_PLUS_PI_OVER_2 * slat) / (1.0 + cos(theta));
    theta += delta_theta;
    count--;
  }
  ...
  double northing =
      2.0 * Ra_Over_Sqrt_Two_Plus_PI * theta + Eck6_False_Northing;

  return new MapProjectionCoordinates(CoordinateType::eckert6, easting,
                                      northing);
}
...
MSP::CCS::GeodeticCoordinates* Eckert6::convertToGeodetic(
  ...
  if ((easting < (Eck6_False_Easting + Eck6_Min_Easting)) ||
      (easting >
       (Eck6_False_Easting + Eck6_Max_Easting))) { /* Easting out of range  */
    throw CoordinateConversionException(ErrorMessages::easting);
  }
  if ((northing < (Eck6_False_Northing - Eck6_Delta_Northing)) ||
      (northing > (Eck6_False_Northing +
                   Eck6_Delta_Northing))) { /* Northing out of range */
