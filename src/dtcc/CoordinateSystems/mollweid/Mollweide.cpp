...
Mollweide::Mollweide(double ellipsoidSemiMajorAxis, double ellipsoidFlattening,
...
MapProjection3Parameters* Mollweide::getParameters() const {
  ...
  return new MapProjection3Parameters(CoordinateType::mollweide,
                                      Moll_Origin_Long, Moll_False_Easting,
                                      Moll_False_Northing);
}

MSP::CCS::MapProjectionCoordinates* Mollweide::convertFromGeodetic(
  ...
  while (fabs(delta_theta_primed) > dtp_tolerance && count) {
    delta_theta_primed = -(theta_primed + sin(theta_primed) - PI_Sin_Latitude) /
                         (1.0 + cos(theta_primed));
  ...
  double easting = (Sqrt8_Ra / PI) * dlam * cos(theta) + Moll_False_Easting;
  double northing = Sqrt2_Ra * sin(theta) + Moll_False_Northing;

  return new MapProjectionCoordinates(CoordinateType::mollweide, easting,
                                      northing);
}

MSP::CCS::GeodeticCoordinates* Mollweide::convertToGeodetic(
  ...
  if ((easting < (Moll_False_Easting + Moll_Min_Easting)) ||
      (easting >
       (Moll_False_Easting + Moll_Max_Easting))) { /* Easting out of range  */
    throw CoordinateConversionException(ErrorMessages::easting);
  }
  if ((northing < (Moll_False_Northing - Moll_Delta_Northing)) ||
      (northing > (Moll_False_Northing +
                   Moll_Delta_Northing))) { /* Northing out of range */
  ...
  if (fabs(fabs(latitude) - MAX_LAT) < 1.0e-10)
    ...
    longitude = Moll_Origin_Long + PI * dx / (Sqrt8_Ra * cos(theta));
