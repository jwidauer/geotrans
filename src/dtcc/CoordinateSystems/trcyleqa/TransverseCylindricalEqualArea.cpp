 ...
 * REUSE NOTES
 *
 *    TRANSVERSE CYLINDRICAL EQUAL AREA is intended for reuse by any application
 * that performs a Transverse Cylindrical Equal Area projection or its inverse.
 *
 * REFERENCES
 *
 *    Further information on TRANSVERSE CYLINDRICAL EQUAL AREA can be found in
 * the Reuse Manual.
 *
 ...
 * ENVIRONMENT
 *
 *    TRANSVERSE CYLINDRICAL EQUAL AREA was tested and certified in the
...
TransverseCylindricalEqualArea::TransverseCylindricalEqualArea(
   ...
   * : Longitude in radians at the center of     (input) the projection
   *    latitudeOfTrueScale    : Latitude in radians at which the (input) point
  ...
  if ((latitudeOfTrueScale < -PI_OVER_2) ||
      (latitudeOfTrueScale > PI_OVER_2)) { /* origin latitude out of range */
...
MapProjection5Parameters* TransverseCylindricalEqualArea::getParameters()
    const {
  /*
   * The function getParameters returns the current ellipsoid
   * parameters, Transverse Cylindrical Equal Area projection parameters, and
   * scale factor.
   *
   *  ellipsoidSemiMajorAxis  : Semi-major axis of ellipsoid, in meters (output)
   *  ellipsoidFlattening     : Flattening of ellipsoid (output) centralMeridian
   * : Longitude in radians at the center of     (output) the projection
   *  latitudeOfTrueScale     : Latitude in radians at which the (output) point
   * scale factor is 1.0 falseEasting            : A coordinate value in meters
   * assigned to the central meridian of the projection.       (output)
   *  falseNorthing           : A coordinate value in meters assigned to the
  ...
  return new MapProjection5Parameters(
      CoordinateType::transverseCylindricalEqualArea, Tcea_Origin_Long,
      Tcea_Origin_Lat, Tcea_Scale_Factor, Tcea_False_Easting,
      Tcea_False_Northing);
}

MSP::CCS::MapProjectionCoordinates*
TransverseCylindricalEqualArea::convertFromGeodetic(
    MSP::CCS::GeodeticCoordinates* geodeticCoordinates) {
  /*
   * The function convertFromGeodetic converts geodetic (latitude and
   * longitude) coordinates to Transverse Cylindrical Equal Area projection
   * (easting and northing) coordinates, according to the current ellipsoid and
   * Transverse Cylindrical Equal Area projection parameters.  If any errors
  ...
  char warning[100];
  warning[0] = '\0';
  if (fabs(dlam) >= (PI / 2)) { /* Distortion resultsif Longitude is > 90 deg
  ...
  double easting = semiMajorAxis * cos(beta) * cos(PHIc) * sin(dlam) /
                       (Tcea_Scale_Factor * cos(betac) *
                        sqrt(1.0 - es2 * sinPHIc * sinPHIc)) +
                   Tcea_False_Easting;
  ...
  return new MapProjectionCoordinates(
      CoordinateType::transverseCylindricalEqualArea, warning, easting,
      northing);
}

MSP::CCS::GeodeticCoordinates*
TransverseCylindricalEqualArea::convertToGeodetic(
  ...
  if ((easting < (Tcea_False_Easting + Tcea_Min_Easting)) ||
      (easting >
       (Tcea_False_Easting + Tcea_Max_Easting))) { /* Easting out of range */
    throw CoordinateConversionException(ErrorMessages::easting);
  }
  if ((northing < (Tcea_False_Northing + Tcea_Min_Northing)) ||
      (northing >
       (Tcea_False_Northing + Tcea_Max_Northing))) { /* Northing out of range */
  ...
  cosbetac = cos(betac);
  temp = Tcea_Scale_Factor * dx * cosbetac *
         sqrt(1.0 - es2 * sin_lat * sin_lat) / (semiMajorAxis * cos(PHIc));
  if (temp > 1.0)
    temp = 1.0;
  else if (temp < -1.0)
    temp = -1.0;
...
double TransverseCylindricalEqualArea::TCEA_Q(double sinlat, double x) {
  return One_MINUS_es2 * (sinlat / (1.0 - es2 * sinlat * sinlat) -
                          One_OVER_2es * log((1 - x) / (1 + x)));
}

double TransverseCylindricalEqualArea::TCEA_COEFF_TIMES_SIN(double coeff,
                                                            double x,
                                                            double latit) {
  return coeff * sin(x * latit);
}

double TransverseCylindricalEqualArea::TCEA_M(double c0lat, double c1lat,
                                              double c2lat, double c3lat) {
  return semiMajorAxis * (c0lat - c1lat + c2lat - c3lat);
}

double TransverseCylindricalEqualArea::TCEA_L(double Beta, double c0lat,
                                              double c1lat, double c2lat) {
