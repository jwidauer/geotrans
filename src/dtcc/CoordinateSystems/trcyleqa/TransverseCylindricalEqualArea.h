 ...
 *                                      range (0.3 to 3.0)
 *          TCEA_LON_WARNING        : Distortion will result if longitude is
...
class TransverseCylindricalEqualArea : public CoordinateSystem {
 public:
   ...
   * Transverse Cylindrical Equal Area projection parameters as inputs, and sets
   ...
   * (input) the projection latitudeOfTrueScale        : Latitude in radians at
   ...
   * projection.       (input) falseNorthing              : A coordinate value
   ...
   *                                 ellipsoid (input) errorStatus : Error
   * status                              (output)
   */

  TransverseCylindricalEqualArea(double ellipsoidSemiMajorAxis,
