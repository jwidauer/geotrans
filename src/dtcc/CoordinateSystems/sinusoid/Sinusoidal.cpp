...
Sinusoidal::Sinusoidal(double ellipsoidSemiMajorAxis,
   ...
   * Sinusoidal projection parameters as inputs, and sets the corresponding
   * state variables.  If any errors occur, an exception is thrown with a
...
MapProjection3Parameters* Sinusoidal::getParameters() const {
  ...
  return new MapProjection3Parameters(CoordinateType::sinusoidal,
                                      Sinu_Origin_Long, Sinu_False_Easting,
                                      Sinu_False_Northing);
}
...
MSP::CCS::MapProjectionCoordinates* Sinusoidal::convertFromGeodetic(
  ...
  return new MapProjectionCoordinates(CoordinateType::sinusoidal, easting,
                                      northing);
}

MSP::CCS::GeodeticCoordinates* Sinusoidal::convertToGeodetic(
  ...
  if ((easting < (Sinu_False_Easting + Sinu_Min_Easting)) ||
      (easting >
       (Sinu_False_Easting + Sinu_Max_Easting))) { /* Easting out of range */
    throw CoordinateConversionException(ErrorMessages::easting);
  }
  if ((northing < (Sinu_False_Northing - Sinu_Delta_Northing)) ||
      (northing > (Sinu_False_Northing +
                   Sinu_Delta_Northing))) { /* Northing out of range */
  ...
  if (floatEq(fabs(latitude), PI_OVER_2, 1.0e-8))
    ...
    longitude = Sinu_Origin_Long + dx * sqrt(1.0 - es2 * sin_lat * sin_lat) /
                                       (semiMajorAxis * cos(latitude));
