 ...
 *                                       depending on ellipsoid and projection
 *                                       parameters)
 *          STEREO_A_ERROR            : Semi-major axis less than or equal to
 * zero STEREO_INV_F_ERROR        : Inverse flattening outside of valid range
 *                                      (250 to 350)
 *
 *
 * REUSE NOTES
...
Stereographic::Stereographic(double ellipsoidSemiMajorAxis,
   ...
   * the center of          (input) the projection falseEasting              :
   * Easting (X) at center of projection, in meters  (input) falseNorthing :
   * Northing (Y) at center of projection, in meters (input)
   */
  ...
  if ((Stereo_At_Pole) || (fabs(Stereo_Origin_Lat) < 1.0e-10)) {
    ...
    if (Stereo_Origin_Long <= 0) {
      GeodeticCoordinates gcTemp(CoordinateType::geodetic,
                                 PI + Stereo_Origin_Long - ONE,
                                 -Stereo_Origin_Lat);
      tempCoordinates = convertFromGeodetic(&gcTemp);
    } else {
      GeodeticCoordinates gcTemp(CoordinateType::geodetic,
                                 Stereo_Origin_Long - PI - ONE,
                                 -Stereo_Origin_Lat);
      tempCoordinates = convertFromGeodetic(&gcTemp);
    }
    ...
    if (Stereo_False_Easting) Stereo_Delta_Easting -= Stereo_False_Easting;
...
MapProjection4Parameters* Stereographic::getParameters() const {
   ...
   * the center of    (output) the projection falseEasting            : A
  ...
  return new MapProjection4Parameters(
      CoordinateType::stereographic, Stereo_Origin_Long, Stereo_Origin_Lat,
      Stereo_False_Easting, Stereo_False_Northing);
}

MSP::CCS::MapProjectionCoordinates* Stereographic::convertFromGeodetic(
  ...
  cos_dlam = cos(dlam);
  g = 1.0 + Sin_Stereo_Origin_Lat * slat +
      Cos_Stereo_Origin_Lat * clat * cos_dlam;
  if (fabs(g) <=
      1.0e-10) { /* Point is out of view.  Will return longitude out of range
                    message since no point out of view is implemented.  */
    throw CoordinateConversionException(ErrorMessages::longitude);
  } else {
    if (Stereo_At_Pole) {
      ...
      if (fabs(Stereo_Origin_Lat) <= 1.0e-10) {
        ...
        Ra_k = Stereo_Ra * k;
        northing = Stereo_False_Northing +
                   Ra_k * (Cos_Stereo_Origin_Lat * slat -
                           Sin_Stereo_Origin_Lat * clat * cos_dlam);
      }
      easting = Stereo_False_Easting + Ra_k * clat * sin(dlam);
    }
  }

  return new MapProjectionCoordinates(CoordinateType::stereographic, easting,
                                      northing);
}

MSP::CCS::GeodeticCoordinates* Stereographic::convertToGeodetic(
   ...
   * coordinates, according to the current ellipsoid and Stereographic
  ...
  if ((easting < (Stereo_False_Easting - Stereo_Delta_Easting)) ||
      (easting > (Stereo_False_Easting +
                  Stereo_Delta_Easting))) { /* Easting out of range  */
    throw CoordinateConversionException(ErrorMessages::easting);
  }
  if ((northing < (Stereo_False_Northing - Stereo_Delta_Northing)) ||
      (northing > (Stereo_False_Northing +
                   Stereo_Delta_Northing))) { /* Northing out of range */
  ...
  if (fabs(rho) <= 1.0e-10) {
    ...
    if (Stereo_At_Pole) {
      ...
      longitude = Stereo_Origin_Long +
                  atan2(dx * sin_c, (rho * Cos_Stereo_Origin_Lat * cos_c -
                                     dy_sin_c * Sin_Stereo_Origin_Lat));
    latitude = asin(cos_c * Sin_Stereo_Origin_Lat +
                    ((dy_sin_c * Cos_Stereo_Origin_Lat) / rho));
  }
