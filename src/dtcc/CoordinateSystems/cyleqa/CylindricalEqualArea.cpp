...
CylindricalEqualArea::CylindricalEqualArea(double ellipsoidSemiMajorAxis,
  ...
  k0 = cos(Cyeq_Origin_Lat) /
       sqrt(1.0 - es2 * Sin_Cyeq_Origin_Lat * Sin_Cyeq_Origin_Lat);
  Cyeq_a_k0 = semiMajorAxis * k0;
  two_k0 = 2.0 * k0;

  if (Cyeq_Origin_Long > 0) {
    GeodeticCoordinates gcMax(CoordinateType::geodetic,
                              Cyeq_Origin_Long - PI - ONE, PI_OVER_2);
    ...
    GeodeticCoordinates gcMin(CoordinateType::geodetic, Cyeq_Origin_Long - PI,
                              PI_OVER_2);
    ...
    GeodeticCoordinates gcMax(CoordinateType::geodetic, Cyeq_Origin_Long + PI,
                              PI_OVER_2);
    MapProjectionCoordinates* tempCoordinates = convertFromGeodetic(&gcMax);
    Cyeq_Max_Easting = tempCoordinates->easting();
    delete tempCoordinates;
  ...
  if (Cyeq_False_Northing) Cyeq_Delta_Northing -= Cyeq_False_Northing;
  if (Cyeq_Delta_Northing < 0) Cyeq_Delta_Northing = -Cyeq_Delta_Northing;
...
MapProjection4Parameters* CylindricalEqualArea::getParameters() const {
  ...
  return new MapProjection4Parameters(CoordinateType::cylindricalEqualArea,
                                      Cyeq_Origin_Long, Cyeq_Origin_Lat,
                                      Cyeq_False_Easting, Cyeq_False_Northing);
}

MSP::CCS::MapProjectionCoordinates* CylindricalEqualArea::convertFromGeodetic(
...
MSP::CCS::GeodeticCoordinates* CylindricalEqualArea::convertToGeodetic(
  ...
  if ((easting < (Cyeq_False_Easting + Cyeq_Min_Easting)) ||
      (easting >
       (Cyeq_False_Easting + Cyeq_Max_Easting))) { /* Easting out of range */
    throw CoordinateConversionException(ErrorMessages::easting);
  }
  if ((northing < (Cyeq_False_Northing - fabs(Cyeq_Delta_Northing))) ||
      (northing > (Cyeq_False_Northing +
                   fabs(Cyeq_Delta_Northing)))) { /* Northing out of range */
...
double CylindricalEqualArea::cyleqarQ(double slat, double x) {
  return (1.0 - es2) * (slat / (1.0 - x * x) -
                        (1.0 / (2.0 * es)) * log((1.0 - x) / (1.0 + x)));
}
