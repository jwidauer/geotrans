...
VanDerGrinten::VanDerGrinten(double ellipsoidSemiMajorAxis,
...
MapProjection3Parameters* VanDerGrinten::getParameters() const {
  ...
  return new MapProjection3Parameters(CoordinateType::vanDerGrinten,
                                      Grin_Origin_Long, Grin_False_Easting,
                                      Grin_False_Northing);
}

MSP::CCS::MapProjectionCoordinates* VanDerGrinten::convertFromGeodetic(
  ...
  if (latitude == 0.0) {
    ...
    northing = 0.0;
  } else if (dlam == 0.0 || floatEq(latitude, MAX_LAT, .00001) ||
             floatEq(latitude, -MAX_LAT, .00001)) {
    ...
    easting = PI_Ra *
                  ...
                  (ppsqr_PLUS_aasqr) +
              Grin_False_Easting;
    if (dlam < 0.0) easting *= -1.0;
    northing =
        PI_Ra *
            (pp * qq - aa * sqrt((aasqr + 1) * (ppsqr_PLUS_aasqr)-qq * qq)) /
            (ppsqr_PLUS_aasqr) +
        Grin_False_Northing;
    if (latitude < 0.0) northing *= -1.0;
  }

  return new MapProjectionCoordinates(CoordinateType::vanDerGrinten, easting,
                                      northing);
}

MSP::CCS::GeodeticCoordinates* VanDerGrinten::convertToGeodetic(
  ...
  if ((easting > (Grin_False_Easting + delta)) ||
      (easting < (Grin_False_Easting - delta))) { /* Easting out of range */
    throw CoordinateConversionException(ErrorMessages::easting);
  }
  if ((northing > (Grin_False_Northing + delta)) ||
      (northing < (Grin_False_Northing - delta))) { /* Northing out of range */
  ...
  if ((temp > (Grin_False_Easting + PI_Ra + epsilon)) ||
      ...
      (temp < (Grin_False_Northing - PI_Ra -
               epsilon))) { /* Point is outside of projection area */
  ...
  if (northing == 0.0)
    ...
    c3sqr = c3 * c3;
    dd = yysqr / c3 +
         ((2 * c2 * c2 * c2) / (c3sqr * c3) - (9 * c1 * c2) / (c3sqr)) / 27;
  ...
  if (xx == 0.0)
    ...
    longitude = PI *
                    (xxsqr_PLUS_yysqr - 1 +
                     sqrt(1 + (2 * xxsqr - two_yysqr) +
                          (xxsqr_PLUS_yysqr) * (xxsqr_PLUS_yysqr))) /
                    (2 * xx) +
                Grin_Origin_Long;
  }
