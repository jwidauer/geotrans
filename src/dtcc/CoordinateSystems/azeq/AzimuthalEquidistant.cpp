...
AzimuthalEquidistant::AzimuthalEquidistant(double ellipsoidSemiMajorAxis,
  ...
  if (fabs(abs_Azeq_Origin_Lat - PI_OVER_2) < 1.0e-10) {
    ...
    if (Azeq_Origin_Long > 0.0) {
      GeodeticCoordinates gcDelta(CoordinateType::geodetic,
                                  Azeq_Origin_Long - PI + ONE,
                                  -Azeq_Origin_Lat);
    ...
    if (Azeq_False_Easting) Azeq_Delta_Easting -= Azeq_False_Easting;
    if (Azeq_Delta_Easting < 0) Azeq_Delta_Easting = -Azeq_Delta_Easting;
...
MapProjection4Parameters* AzimuthalEquidistant::getParameters() const {
  ...
  return new MapProjection4Parameters(CoordinateType::azimuthalEquidistant,
                                      Azeq_Origin_Long, Azeq_Origin_Lat,
                                      Azeq_False_Easting, Azeq_False_Northing);
}

MSP::CCS::MapProjectionCoordinates* AzimuthalEquidistant::convertFromGeodetic(
  ...
  if (fabs(abs_Azeq_Origin_Lat - PI_OVER_2) < 1.0e-10) {
    ...
    if (fabs(fabs(cos_c) - 1.0) < 1.0e-14) {
      if (cos_c >= 0.0) {
        ...
        throw CoordinateConversionException(
            MSP::CCS::ErrorMessages::projection);
      }
    } else {
      c = acos(cos_c);
      k_prime = c / sin(c);
    ...
    cos_c =
        (Sin_Azeq_Origin_Lat * slat) + (Cos_Azeq_Origin_Lat * clat * cos_dlam);
    if (fabs(fabs(cos_c) - 1.0) < 1.0e-14) {
      if (cos_c >= 0.0) {
        easting = Azeq_False_Easting;
        northing = Azeq_False_Northing;
      ...
      Ra_kprime = Ra * k_prime;
      easting = Ra_kprime * clat * sin_dlam + Azeq_False_Easting;
      northing = Ra_kprime * (Cos_Azeq_Origin_Lat * slat -
                              Sin_Azeq_Origin_Lat * clat * cos_dlam) +
                 Azeq_False_Northing;
    }
  }
...
MSP::CCS::GeodeticCoordinates* AzimuthalEquidistant::convertToGeodetic(
  ...
  if ((easting < (Azeq_False_Easting - Azeq_Delta_Easting)) ||
      (easting >
       (Azeq_False_Easting + Azeq_Delta_Easting))) { /* Easting out of range  */
    throw CoordinateConversionException(ErrorMessages::easting);
  }
  if ((northing < (Azeq_False_Northing - Azeq_Delta_Northing)) ||
      (northing > (Azeq_False_Northing +
                   Azeq_Delta_Northing))) { /* Northing out of range */
  ...
  if (fabs(rho) <= 1.0e-10) {
    ...
    cos_c = cos(c);
    dy_sinc = dy * sin_c;
    latitude = asin((cos_c * Sin_Azeq_Origin_Lat) +
                    ((dy_sinc * Cos_Azeq_Origin_Lat) / rho));
    if (fabs(abs_Azeq_Origin_Lat - PI_OVER_2) < 1.0e-10) {
      if (Azeq_Origin_Lat >= 0.0)
      ...
      longitude = Azeq_Origin_Long +
                  atan2((dx * sin_c), ((rho * Cos_Azeq_Origin_Lat * cos_c) -
                                       (dy_sinc * Sin_Azeq_Origin_Lat)));
  }
