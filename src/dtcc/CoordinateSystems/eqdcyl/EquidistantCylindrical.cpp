 ...
 *    (latitude and longitude in radians) and Equidistant Cylindrical projection
 * coordinates (easting and northing in meters).  The Equidistant Cylindrical
 ...
 * REUSE NOTES
 *
 *    EQUIDISTANT CYLINDRICAL is intended for reuse by any application that
 * performs a Equidistant Cylindrical projection or its inverse.
 *
 * REFERENCES
 *
 *    Further information on EQUIDISTANT CYLINDRICAL can be found in the Reuse
 * Manual.
 *
 *    EQUIDISTANT CYLINDRICAL originated from :  U.S. Army Topographic
 ...
 * ENVIRONMENT
 *
 *    EQUIDISTANT CYLINDRICAL was tested and certified in the following
 * environments:
 *
...
EquidistantCylindrical::EquidistantCylindrical(double ellipsoidSemiMajorAxis,
   ...
   *                              the projection
   *    standardParallel        : Latitude in radians at which the (input) point
  ...
  if ((standardParallel < -PI_OVER_2) ||
      (standardParallel > PI_OVER_2)) { /* standard parallel out of range */
  ...
  if (Eqcy_Origin_Long > 0) {
    GeodeticCoordinates gcMax(CoordinateType::geodetic,
                              Eqcy_Origin_Long - PI - ONE, PI_OVER_2);
    ...
    GeodeticCoordinates gcMin(CoordinateType::geodetic, Eqcy_Origin_Long - PI,
                              PI_OVER_2);
    tempCoordinates = convertFromGeodetic(&gcMin);
    Eqcy_Min_Easting = tempCoordinates->easting();
    delete tempCoordinates;
  } else if (Eqcy_Origin_Long < 0) {
    GeodeticCoordinates gcMax(CoordinateType::geodetic, Eqcy_Origin_Long + PI,
                              PI_OVER_2);
    MapProjectionCoordinates* tempCoordinates = convertFromGeodetic(&gcMax);
    Eqcy_Max_Easting = tempCoordinates->easting();
    delete tempCoordinates;

    GeodeticCoordinates gcMin(CoordinateType::geodetic,
                              Eqcy_Origin_Long + PI + ONE, PI_OVER_2);
    tempCoordinates = convertFromGeodetic(&gcMin);
    Eqcy_Min_Easting = tempCoordinates->easting();
    delete tempCoordinates;
...
EquidistantCylindricalParameters* EquidistantCylindrical::getParameters()
    const {
  /*
  ...
  return new EquidistantCylindricalParameters(
      CoordinateType::equidistantCylindrical, Eqcy_Origin_Long,
      Eqcy_Std_Parallel, Eqcy_False_Easting, Eqcy_False_Northing);
}

MSP::CCS::MapProjectionCoordinates* EquidistantCylindrical::convertFromGeodetic(
    MSP::CCS::GeodeticCoordinates* geodeticCoordinates) {
  /*
   * The function convertFromGeodetic converts geodetic (latitude and
   * longitude) coordinates to Equidistant Cylindrical projection (easting and
   * northing) coordinates, according to the current ellipsoid, spherical
   * radiius and Equidistant Cylindrical projection parameters. If any errors
...
MSP::CCS::GeodeticCoordinates* EquidistantCylindrical::convertToGeodetic(
  ...
  if ((easting < (Eqcy_False_Easting + Eqcy_Min_Easting)) ||
      (easting >
       (Eqcy_False_Easting + Eqcy_Max_Easting))) { /* Easting out of range */
    throw CoordinateConversionException(ErrorMessages::easting);
  }
  if ((northing < (Eqcy_False_Northing - Eqcy_Delta_Northing)) ||
      (northing > (Eqcy_False_Northing +
                   Eqcy_Delta_Northing))) { /* Northing out of range */
