...
const double MIN_USNG_NON_POLAR_LAT =
    -80.0 * (PI / 180.0); /* -80 degrees in radians    */
const double MAX_USNG_NON_POLAR_LAT =
    84.0 * (PI / 180.0); /* 84 degrees in radians     */
...
void makeUSNGString(char* USNGString, long zone, int letters[USNG_LETTERS],
  ...
  for (j = 0; j < 3; j++) USNGString[i++] = alphabet[letters[j]];
...
void breakUSNGString(char* USNGString, long* zone, long letters[USNG_LETTERS],
  ...
  while (USNGString[i] == ' ') i++; /* skip any leading blanks */
  j = i;
  while (isdigit(USNGString[i])) i++;
  num_digits = i - j;
  if (num_digits <= 2)
    if (num_digits > 0) {
      char zone_string[3];
...
USNG::USNG(double ellipsoidSemiMajorAxis, double ellipsoidFlattening,
           char* ellipsoidCode)
    : CoordinateSystem(), ups(0), utm(0) {
  /*
   ...
   *   ellipsoidFlattening      : Flattening of ellipsoid (input) ellipsoid_Code
   * : 2-letter code for ellipsoid             (input)
   */
...
MSP::CCS::MGRSorUSNGCoordinates* USNG::convertFromGeodetic(
  ...
  if ((longitude < -PI) ||
      (longitude > (2 * PI))) { /* longitude out of range */
  ...
  try {
    if ((latitude >= MIN_USNG_NON_POLAR_LAT) &&
      ...
      mgrsorUSNGCoordinates =
          fromUTM(utmCoordinates, longitude, latitude, precision);
...
MSP::CCS::GeodeticCoordinates* USNG::convertToGeodetic(
  ...
  breakUSNGString(usngCoordinates->MGRSString(), &zone, letters, &usng_easting,
                  &usng_northing, &precision);

  try {
    if (zone) {
      utmCoordinates =
          toUTM(zone, letters, usng_easting, usng_northing, precision);
...
MSP::CCS::MGRSorUSNGCoordinates* USNG::convertFromUTM(
  ...
  GeodeticCoordinates* geodeticCoordinates =
      utm->convertToGeodetic(utmCoordinates);
  ...
  try {
    if ((latitude >= (MIN_USNG_NON_POLAR_LAT - EPSILON)) &&
      ...
      mgrsorUSNGCoordinates =
          fromUTM(utmCoordinates, geodeticCoordinates->longitude(), latitude,
                  precision);
    else {
      upsCoordinates = ups->convertFromGeodetic(geodeticCoordinates);
      mgrsorUSNGCoordinates = fromUPS(upsCoordinates, precision);
...
MSP::CCS::UTMCoordinates* USNG::convertToUTM(
  ...
  breakUSNGString(mgrsorUSNGCoordinates->MGRSString(), &zone, letters,
                  &usng_easting, &usng_northing, &precision);
  try {
    if (zone) {
      utmCoordinates =
          toUTM(zone, letters, usng_easting, usng_northing, precision);
      // Convert to geodetic to make sure the coordinates are in the valid utm
      // range
      geodeticCoordinates = utm->convertToGeodetic(utmCoordinates);
    } else {
      upsCoordinates = toUPS(letters, usng_easting, usng_northing);
...
MSP::CCS::MGRSorUSNGCoordinates* USNG::convertFromUPS(
  ...
  GeodeticCoordinates* geodeticCoordinates =
      ups->convertToGeodetic(upsCoordinates);
...
MSP::CCS::MGRSorUSNGCoordinates* USNG::fromUTM(
    MSP::CCS::UTMCoordinates* utmCoordinates, double longitude, double latitude,
    long precision) {
  /*
  ...
  if (natural_zone > 60) natural_zone = 1;

  if (zone != natural_zone) {  // reconvert to override zone
    UTM utmOverride(semiMajorAxis, flattening, USNGEllipsoidCode, natural_zone);
    GeodeticCoordinates geodeticCoordinates(CoordinateType::geodetic, longitude,
                                            latitude);
  ...
  if (letters[0] == LETTER_V)  // V latitude band
  {
    ...
    if ((zone == 32) && (easting < _500000))  // extension of zone 31X
      override = 31;
              ...
              (easting >= _500000)) ||  // western extension of zone 33X
             ((zone == 34) &&
              (easting < _500000)))  // eastern extension of zone 33X
      override = 33;
    else if (((zone == 34) &&
              (easting >= _500000)) ||  // western extension of zone 35X
             ((zone == 36) &&
              (easting < _500000)))  // eastern extension of zone 35X
      override = 35;
    else if ((zone == 36) &&
             (easting >= _500000))  // western extension of zone 37X
      override = 37;
  }

  if (override) {  // reconvert to override zone
...
MSP::CCS::UTMCoordinates* USNG::toUTM(long zone, long letters[USNG_LETTERS],
  ...
  if ((letters[0] == LETTER_X) &&
    ...
    utmCoordinates =
        new UTMCoordinates(CoordinateType::universalTransverseMercator, zone,
...
MSP::CCS::UPSCoordinates* USNG::toUPS(long letters[USNG_LETTERS],
                                      double easting, double northing) {
  /*
  ...
  if (letters[2] > LETTER_I) grid_northing = grid_northing - ONEHT;

  if (letters[2] > LETTER_O) grid_northing = grid_northing - ONEHT;

  grid_easting =
      (double)((letters[1]) - ltr2_low_value) * ONEHT + false_easting;
  if (ltr2_low_value != LETTER_A) {
    if (letters[1] > LETTER_L) grid_easting = grid_easting - 300000.0;

    if (letters[1] > LETTER_U) grid_easting = grid_easting - 200000.0;
  } else {
    if (letters[1] > LETTER_C) grid_easting = grid_easting - 200000.0;

    if (letters[1] > LETTER_I) grid_easting = grid_easting - ONEHT;
...
void USNG::getGridValues(long zone, long* ltr2_low_value, long* ltr2_high_value,
                         double* pattern_offset) {
  /*
...
void USNG::getLatitudeBandMinNorthing(long letter, double* min_northing,
                                      double* northing_offset) {
  /*
