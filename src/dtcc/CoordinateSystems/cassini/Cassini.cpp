...
Cassini::Cassini(double ellipsoidSemiMajorAxis, double ellipsoidFlattening,
  ...
  if (Cass_Origin_Long > 0) {
    GeodeticCoordinates gcMax(CoordinateType::geodetic, Cass_Origin_Long - PI,
                              THIRTY_ONE);
    ...
    GeodeticCoordinates gcMin(CoordinateType::geodetic, Cass_Origin_Long - PI,
                              -THIRTY_ONE);
    ...
    GeodeticCoordinates gcMax(CoordinateType::geodetic, PI + Cass_Origin_Long,
                              THIRTY_ONE);
    MapProjectionCoordinates* tempCoordinates = convertFromGeodetic(&gcMax);
    Cass_Max_Northing = tempCoordinates->northing();
    delete tempCoordinates;

    GeodeticCoordinates gcMin(CoordinateType::geodetic, PI + Cass_Origin_Long,
                              -THIRTY_ONE);
    tempCoordinates = convertFromGeodetic(&gcMin);
    Cass_Min_Northing = tempCoordinates->northing();
    delete tempCoordinates;
...
MapProjection4Parameters* Cassini::getParameters() const {
  ...
  return new MapProjection4Parameters(CoordinateType::cassini, Cass_Origin_Long,
                                      Cass_Origin_Lat, Cass_False_Easting,
                                      Cass_False_Northing);
}

MSP::CCS::MapProjectionCoordinates* Cassini::convertFromGeodetic(
  ...
  char warning[100];
  warning[0] = '\0';
  if (fabs(dlam) > (4.0 * PI / 180)) { /* Distortion results if Longitude is > 4
                                          deg from the Central Meridian */
  ...
  double easting =
      NN * (AA - (TT * A3 / 6.0) - (8.0 - TT + 8.0 * CC) * (TT * A5 / 120.0)) +
      Cass_False_Easting;
  double northing =
      MM - M0 + NN * tlat * ((A2 / 2.0) + (5.0 - TT + 6.0 * CC) * A4 / 24.0) +
      Cass_False_Northing;

  return new MapProjectionCoordinates(CoordinateType::cassini, warning, easting,
                                      northing);
}
...
MSP::CCS::GeodeticCoordinates* Cassini::convertToGeodetic(
  ...
  if ((easting < (Cass_False_Easting + Cass_Min_Easting)) ||
      (easting >
       (Cass_False_Easting + Cass_Max_Easting))) { /* Easting out of range */
    throw CoordinateConversionException(ErrorMessages::easting);
  }
  if ((northing < (Cass_False_Northing + Cass_Min_Northing - epsilon)) ||
      (northing > (Cass_False_Northing + Cass_Max_Northing +
                   epsilon))) { /* Northing out of range */
  ...
  char warning[100];
  warning[0] = '\0';
  if (fabs(longitude - Cass_Origin_Long) >
      (4.0 * PI / 180)) { /* Distortion results if Longitude is > 4 degr from
...
double Cassini::cassM(double c0lat, double c1s2lat, double c2s4lat,
                      double c3s6lat) {
