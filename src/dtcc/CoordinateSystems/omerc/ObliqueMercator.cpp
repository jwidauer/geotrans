 ...
 *                                     range (0.3 to 3.0)
 *       OMERC_LON_WARNING        : Distortion will result if longitude is 90
 * degrees or more from the Central Meridian
 *
 * REUSE NOTES
 *
 ...
 *
 *    OBLIQUE MERCATOR originated from:     U.S. Army Topographic Engineering
...
ObliqueMercator::ObliqueMercator(double ellipsoidSemiMajorAxis,
                                 double ellipsoidFlattening,
                                 double originLatitude, double longitude1,
                                 double latitude1, double longitude2,
                                 double latitude2, double falseEasting,
   ...
   *    longitude1               : Longitude, in radians, of first point lying
   * on central line                             (input) latitude1 : Latitude,
   * in radians, of first point lying on central line (input) longitude2 :
   * Longitude, in radians, of second point lying on central line (input)
   *    latitude2                : Latitude, in radians, of second point lying
   * on central line                             (input) falseEasting : A
  ...
  if ((originLatitude <= -PI_OVER_2) ||
      (originLatitude >=
       PI_OVER_2)) { /* origin latitude out of range -  can not be at a pole */
    throw CoordinateConversionException(ErrorMessages::originLatitude);
  }
  if ((latitude1 <= -PI_OVER_2) ||
      (latitude1 >=
       PI_OVER_2)) { /* first latitude out of range -  can not be at a pole */
    throw CoordinateConversionException(ErrorMessages::latitude1);
  }
  if ((latitude2 <= -PI_OVER_2) ||
      (latitude2 >=
       PI_OVER_2)) { /* second latitude out of range -  can not be at a pole */
    throw CoordinateConversionException(ErrorMessages::latitude2);
  }
  if (latitude1 == 0.0) { /* first latitude can not be at the equator */
    throw CoordinateConversionException(ErrorMessages::latitude1);
  }
  if (latitude1 ==
      latitude2) { /* first and second latitudes can not be equal */
    throw CoordinateConversionException(ErrorMessages::latitude2);
  }
  if (((latitude1 < 0.0) && (latitude2 > 0.0)) ||
      ((latitude1 > 0.0) &&
       (latitude2 < 0.0))) { /*first and second points can not be in different
                                hemispheres */
    throw CoordinateConversionException(ErrorMessages::omercHemisphere);
  }
  if ((longitude1 < -PI) ||
      (longitude1 > TWO_PI)) { /* first longitude out of range */
    throw CoordinateConversionException(ErrorMessages::longitude1);
  }
  if ((longitude2 < -PI) ||
      (longitude2 > TWO_PI)) { /* first longitude out of range */
    throw CoordinateConversionException(ErrorMessages::longitude2);
  }
  if ((scaleFactor < MIN_SCALE_FACTOR) ||
      (scaleFactor > MAX_SCALE_FACTOR)) { /* scale factor out of range */
  ...
  OMerc_A =
      (semiMajorAxis * OMerc_B * OMerc_Scale_Factor * sqrt(one_MINUS_es2)) /
      (1.0 - es2_sin_olat2);
  ...
  if (dlon < -PI) OMerc_Lon_2 -= TWO_PI;
  if (dlon > PI) OMerc_Lon_2 += TWO_PI;
  dlon = OMerc_Lon_1 - OMerc_Lon_2;
  OMerc_Origin_Long = (OMerc_Lon_1 + OMerc_Lon_2) / 2.0 -
                      (atan(J * tan(OMerc_B * dlon / 2.0) / P)) / OMerc_B;

  dlon = OMerc_Lon_1 - OMerc_Origin_Long;
  if (dlon < -PI) OMerc_Origin_Long -= TWO_PI;
  if (dlon > PI) OMerc_Origin_Long += TWO_PI;
...
ObliqueMercatorParameters* ObliqueMercator::getParameters() const {
   ...
   *                              central line (output) latitude1 : Latitude, in
   * radians, of first point lying on central line (output) longitude2 :
   * Longitude, in radians, of second point lying on central line (output)
   *    latitude2               : Latitude, in radians, of second point lying on
   *                              central line (output) falseEasting : A
   * coordinate value, in meters, assigned to the central meridian of the
  ...
  return new ObliqueMercatorParameters(
      CoordinateType::obliqueMercator, OMerc_Origin_Lat, OMerc_Lon_1,
      OMerc_Lat_1, OMerc_Lon_2, OMerc_Lat_2, OMerc_False_Easting,
      OMerc_False_Northing, OMerc_Scale_Factor);
}

MSP::CCS::MapProjectionCoordinates* ObliqueMercator::convertFromGeodetic(
   ...
   * northing) coordinates, according to the current ellipsoid and Oblique
   * Mercator projection parameters.  If any errors occur, an exception is
  ...
  char warning[256];
  warning[0] = '\0';
  if (fabs(dlam) >= PI_OVER_2) { /* Distortion will result if Longitude is 90
                                    degrees or more from the Central Meridian */
  ...
  if (fabs(fabs(latitude) - PI_OVER_2) > 1.0e-10) {
    ...
    if (fabs(fabs(U) - 1.0) < 1.0e-10) { /* Point projects into infinity */
      throw CoordinateConversionException(ErrorMessages::longitude);
    } else {
      v = A_over_B * log((1.0 - U) / (1.0 + U)) / 2.0;
      cos_B_dlam = cos(B_dlam);
      if (fabs(cos_B_dlam) < 1.0e-10) u = OMerc_A * B_dlam;
  ...
  return new MapProjectionCoordinates(CoordinateType::obliqueMercator, warning,
                                      easting, northing);
}

MSP::CCS::GeodeticCoordinates* ObliqueMercator::convertToGeodetic(
  ...
  if ((easting < (OMerc_False_Easting - OMerc_Delta_Easting)) ||
      (easting > (OMerc_False_Easting +
                  OMerc_Delta_Easting))) { /* Easting out of range  */
    throw CoordinateConversionException(ErrorMessages::easting);
  }
  if ((northing < (OMerc_False_Northing - OMerc_Delta_Northing)) ||
      (northing > (OMerc_False_Northing +
                   OMerc_Delta_Northing))) { /* Northing out of range */
  ...
  if (fabs(fabs(U_prime) - 1.0) < 1.0e-10) {
    ...
    longitude =
        OMerc_Origin_Long -
        atan2((S_prime * cos_gamma - V_prime * sin_gamma), cos(u_B_over_A)) /
            OMerc_B;
  }
  ...
  char warning[256];
  warning[0] = '\0';
  if (fabs(longitude - OMerc_Origin_Long) >=
      PI_OVER_2) { /* Distortion results if Longitude > 90 degrees from the
                      Central Meridian */
    strcat(warning, MSP::CCS::WarningMessages::longitude);
  }

  return new GeodeticCoordinates(CoordinateType::geodetic, warning, longitude,
                                 latitude);
}

double ObliqueMercator::omercT(double lat, double e_sinlat, double e_over_2) {
  return (tan(PI_OVER_4 - lat / 2.0)) /
