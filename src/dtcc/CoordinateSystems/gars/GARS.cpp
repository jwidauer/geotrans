 ...
 *                                      string too short, invalid
 * numbers/letters GARS_STR_LAT_ERROR     : The latitude part of the GARS string
 *                                     (fourth and fifth  characters) is
 * invalid. GARS_STR_LON_ERROR     : The longitude part of the GARS string
...
MSP::CCS::GARSCoordinates* GARS::convertFromGeodetic(
   ...
   * in radians) coordinates to a GARS coordinate string.  Precision specifies
   * the number of digits in the GARS string for latitude and longitude: 0: 30
   * minutes (5 characters) 1: 15 minutes (6 characters) 2: 5 minutes (7
   * characters)
   *
  ...
  if (latitude == MAX_LATITUDE) latitude = 89.99999999999;
  ...
  if (letter_index[0] >= LETTER_I) letter_index[0]++;
  if (letter_index[0] >= LETTER_O) letter_index[0]++;

  if (letter_index[1] >= LETTER_I) letter_index[1]++;
  if (letter_index[1] >= LETTER_O) letter_index[1]++;
  ...
  if (ew_value < 10)
    sprintf(GARSString, "00%d%c%c", ew_value,
            (char)(letter_index[0] + LETTER_A_OFFSET),
            (char)(letter_index[1] + LETTER_A_OFFSET));
  else if (ew_value < 100)
    sprintf(GARSString, "0%d%c%c", ew_value,
            (char)(letter_index[0] + LETTER_A_OFFSET),
            (char)(letter_index[1] + LETTER_A_OFFSET));
  else
    sprintf(GARSString, "%d%c%c", ew_value,
            (char)(letter_index[0] + LETTER_A_OFFSET),
            (char)(letter_index[1] + LETTER_A_OFFSET));
  ...
  if (precision > 0) {
    ...
    if (precision > 1) strcat(GARSString, _5_minute_value_str);
  }
...
MSP::CCS::GeodeticCoordinates* GARS::convertToGeodetic(
    MSP::CCS::GARSCoordinates* garsCoordinates) {
  /*
   *  The function convertToGeodetic converts a GARS coordinate string to
  ...
  if (index + 1 < gars_length) {
    ...
    if (!isdigit(_15_minute_value) || _15_minute_value < _1 ||
        _15_minute_value > _4)
      ...
      if (index + 1 < gars_length) {
        ...
        if (!isdigit(_5_minute_value) || _5_minute_value < _1 ||
            _5_minute_value > _9)
  ...
  if (ns_str[0] >= LETTER_O) ns_str[0]--;
  if (ns_str[0] >= LETTER_I) ns_str[0]--;

  if (ns_str[1] >= LETTER_O) ns_str[1]--;
  if (ns_str[1] >= LETTER_I) ns_str[1]--;
