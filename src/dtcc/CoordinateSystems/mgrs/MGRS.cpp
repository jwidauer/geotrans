...
void makeMGRSString(char* MGRSString, long zone, int letters[MGRS_LETTERS],
  ...
  for (j = 0; j < 3; j++) MGRSString[i++] = alphabet[letters[j]];
...
void breakMGRSString(char* MGRSString, long* zone, long letters[MGRS_LETTERS],
  ...
  while (tempMGRSString[i] == ' ') i++; /* skip any leading blanks */
  j = i;
  while (isdigit(tempMGRSString[i])) i++;
  num_digits = i - j;
  if (num_digits <= 2)
    if (num_digits > 0) {
      char zone_string[3];
  ...
  while (isalpha(tempMGRSString[i])) i++;
  num_letters = i - j;
  if (num_letters == 3) {
    /* get letters */
...
MGRS::MGRS(double ellipsoidSemiMajorAxis, double ellipsoidFlattening,
           char* ellipsoidCode)
...
MSP::CCS::MGRSorUSNGCoordinates* MGRS::convertFromGeodetic(
  ...
  if ((longitude < (-PI - EPSILON)) ||
      (longitude > (2 * PI + EPSILON))) { /* longitude out of range */
...
MSP::CCS::GeodeticCoordinates* MGRS::convertToGeodetic(
  ...
  breakMGRSString(mgrsorUSNGCoordinates->MGRSString(), &zone, letters,
                  &mgrs_easting, &mgrs_northing, &precision);

  try {
    if (zone) {
      utmCoordinates =
          toUTM(zone, letters, mgrs_easting, mgrs_northing, precision);
...
MSP::CCS::UTMCoordinates* MGRS::toUTM(long zone, long letters[MGRS_LETTERS],
  ...
  if ((letters[0] == LETTER_X) &&
    ...
    if (!inLatitudeRange(letters[0], latitude, PI_OVER_180 / divisor)) {
      ...
      if (letters[0] == LETTER_X) nextBand = letters[0];

      if (prevBand == LETTER_I || prevBand == LETTER_O) prevBand--;

      if (nextBand == LETTER_I || nextBand == LETTER_O) nextBand++;
...
MSP::CCS::UPSCoordinates* MGRS::toUPS(long letters[MGRS_LETTERS],
                                      double easting, double northing) {
  /*
...
void MGRS::getGridValues(long zone, long* ltr2_low_value, long* ltr2_high_value,
                         double* pattern_offset) {
  /*
...
void MGRS::getLatitudeBandMinNorthing(long letter, double* min_northing,
                                      double* northing_offset) {
  /*
