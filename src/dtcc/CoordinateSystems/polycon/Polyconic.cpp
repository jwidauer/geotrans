...
Polyconic::Polyconic(double ellipsoidSemiMajorAxis, double ellipsoidFlattening,
  ...
  if (Poly_Origin_Long > 0) {
    GeodeticCoordinates gcMax(CoordinateType::geodetic, Poly_Origin_Long - PI,
                              FOURTY_ONE);
    ...
    GeodeticCoordinates gcMin(CoordinateType::geodetic, Poly_Origin_Long - PI,
                              -FOURTY_ONE);
...
MapProjection4Parameters* Polyconic::getParameters() const {
  ...
  return new MapProjection4Parameters(CoordinateType::polyconic,
                                      Poly_Origin_Long, Poly_Origin_Lat,
                                      Poly_False_Easting, Poly_False_Northing);
}
...
MSP::CCS::MapProjectionCoordinates* Polyconic::convertFromGeodetic(
  ...
  if (fabs(dlam) > (PI / 2)) { /* Distortion results if Longitude is > 90 deg
  ...
  if (latitude == 0.0) {
    ...
    northing = MM - M0 + NN_OVER_tlat * (1.0 - cos(EE)) + Poly_False_Northing;
  }

  return new MapProjectionCoordinates(CoordinateType::polyconic, warning,
                                      easting, northing);
}

MSP::CCS::GeodeticCoordinates* Polyconic::convertToGeodetic(
  ...
  if ((easting < (Poly_False_Easting + Poly_Min_Easting)) ||
      (easting >
       (Poly_False_Easting + Poly_Max_Easting))) { /* Easting out of range */
    throw CoordinateConversionException(ErrorMessages::easting);
  }
  if ((northing < (Poly_False_Northing + Poly_Min_Northing)) ||
      (northing >
       (Poly_False_Northing + Poly_Max_Northing))) { /* Northing out of range */
  ...
  if (floatEq(dy, -M0, 1)) {
    ...
    while (fabs(Delta_PHI) > tolerance && count) {
      ...
      Delta_PHI = (AA_Ma * CC + AA_MINUS_Ma - 0.5 * (Ma2_PLUS_BB)*CC) /
                  (es2 * sin2PHI * (Ma2_PLUS_BB - 2.0 * AA_Ma) / 4.0 * CC +
    ...
    if (floatEq(fabs(latitude), PI_OVER_2, .00001) || (latitude == 0))
      ...
      longitude =
          (asin(dx_OVER_Poly_a * CC)) / sin(latitude) + Poly_Origin_Long;
    }
  }
...
double Polyconic::polyM(double c0lat, double c1s2lat, double c2s4lat,
                        double c3s6lat) {
