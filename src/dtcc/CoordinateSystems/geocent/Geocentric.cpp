...
Geocentric::Geocentric(double ellipsoidSemiMajorAxis,
...
MSP::CCS::GeodeticCoordinates *Geocentric::convertToGeodetic(
  ...
  if (Geocent_algorithm == ITERATIVE) {
    ...
    double Geocent_b =
        semiMajorAxis *
        (1 - flattening); /* Semi-minor axis of ellipsoid, in meters */
...
void Geocentric::geocentricToGeodetic(const double x, const double y,
                                      const double z, double &lat, double &lon,
                                      double &ht) {
  ...
  ht = rho * c + z * s -
       equatorial_radius * sqrt(1.0 - eccentricity_squared * s * s);
