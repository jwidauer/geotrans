...
Bonne::Bonne(double ellipsoidSemiMajorAxis, double ellipsoidFlattening,
  ...
  if (Bonn_Origin_Lat == 0.0) {
    ...
    sinusoidal = new Sinusoidal(semiMajorAxis, flattening, Bonn_Origin_Long,
                                Bonn_False_Easting, Bonn_False_Northing);
  } else {
...
MapProjection4Parameters* Bonne::getParameters() const {
  ...
  return new MapProjection4Parameters(CoordinateType::bonne, Bonn_Origin_Long,
                                      Bonn_Origin_Lat, Bonn_False_Easting,
                                      Bonn_False_Northing);
}

MSP::CCS::MapProjectionCoordinates* Bonne::convertFromGeodetic(
  ...
  if (Bonn_Origin_Lat == 0.0)
    ...
    if ((latitude - Bonn_Origin_Lat) == 0.0 &&
        floatEq(fabs(latitude), PI_OVER_2, .00001)) {
      easting = 0.0;
      northing = 0.0;
    } else {
      mm = bonnm(clat, slat);
    ...
    return new MapProjectionCoordinates(CoordinateType::bonne, easting,
                                        northing);
  }
}
...
MSP::CCS::GeodeticCoordinates* Bonne::convertToGeodetic(
  ...
  if ((easting < (Bonn_False_Easting + Bonn_Min_Easting)) ||
      (easting >
       (Bonn_False_Easting + Bonn_Max_Easting))) { /* Easting out of range */
    throw CoordinateConversionException(ErrorMessages::easting);
  }
  if ((northing < (Bonn_False_Northing - Bonn_Delta_Northing)) ||
      (northing > (Bonn_False_Northing +
                   Bonn_Delta_Northing))) { /* Northing out of range */
  ...
  if (Bonn_Origin_Lat == 0.0)
    ...
    if (Bonn_Origin_Lat < 0.0) rho = -rho;
    ...
    if (floatEq(fabs(latitude), PI_OVER_2, .00001)) {
      ...
      longitude = Bonn_Origin_Long +
                  rho * (atan2(dx, am1sin_dy)) / (semiMajorAxis * mm);
    }
    ...
    return new GeodeticCoordinates(CoordinateType::geodetic, longitude,
                                   latitude);
  }
}
...
double Bonne::bonnM(double c0lat, double c1s2lat, double c2s4lat,
                    double c3s6lat) {
