...
class MSP_DTCC_API TransverseMercator : public CoordinateSystem {
 public:
   ...
   */
  TransverseMercator(double ellipsoidSemiMajorAxis, double ellipsoidFlattening,
                     double centralMeridian, double latitudeOfTrueScale,
                     double falseEasting, double falseNorthing,
                     double scaleFactor, char *ellipsoidCode);
 ...
 private:
   ...
   */
  void latLonToNorthingEasting(const double &lat, const double &lon,
                               double &northing, double &easting);

  /**
   * Basic conversion without regard to false easting/northing or origin
   */
  void northingEastingToLatLon(const double &northing, const double &easting,
   ...
   *
   */
  static void generateCoefficients(double invfla, double &n1, double Acoeff[8],
                                   double Bcoeff[8], double &R4oa,
                                   char *ellipsoidCode);
   ...
   */
  static double geodeticLat(double sinChi, double e);

  /**
   * Use trig identities to compute
   * c2kx[k] = cosh(2kX), s2kx[k] = sinh(2kX)   for k = 0 .. 8
   */
  static void computeHyperbolicSeries(double twoX, double c2kx[],
                                      double s2kx[]);
   ...
   */
  void computeTrigSeries(double twoY, double c2ky[], double s2ky[]);
};
}  // namespace CCS
}  // namespace MSP
