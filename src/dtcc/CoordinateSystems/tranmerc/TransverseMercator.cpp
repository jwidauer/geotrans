 ...
 *
 *    1/19/16  A. Layne MSP_DR30125 Updated generateCoefficients to use book
 *values for coefficients based on supplied ellipsoid code. If user defined, use
 *old default computation.
 *
 *
 */
#include <iostream>
...
TransverseMercator::TransverseMercator(
  ...
  if (ellipsCode[0] == '\0') {  // no ellipsoid code
  ...
  if ((latitudeOfTrueScale < -PI_OVER_2) ||
      (latitudeOfTrueScale > PI_OVER_2)) {  // latitudeOfTrueScale out of range
    throw CoordinateConversionException(ErrorMessages::originLatitude);
  }
  if ((centralMeridian < -PI) ||
      (centralMeridian > (2 * PI))) {  // centralMeridian out of range
  ...
  if (TranMerc_Origin_Long > PI) TranMerc_Origin_Long -= (2 * PI);
  ...
  generateCoefficients(invFlattening, n1, TranMerc_aCoeff, TranMerc_bCoeff,
                       R4oa, ellipsCode);
...
MSP::CCS::MapProjection5Parameters *TransverseMercator::getParameters() const {
  return new MapProjection5Parameters(
      CoordinateType::transverseMercator, TranMerc_Origin_Long,
      TranMerc_Origin_Lat, TranMerc_Scale_Factor, TranMerc_False_Easting,
...
MSP::CCS::MapProjectionCoordinates *TransverseMercator::convertFromGeodetic(
  ...
  latLonToNorthingEasting(TranMerc_Origin_Lat, TranMerc_Origin_Long,
                          falseNorthing, falseEasting);
  ...
  if (invFlattening < 290.0 || invFlattening > 301.0)
    strcat(warning,
           "Eccentricity is outside range that algorithm accuracy has been "
           "tested.");
...
void TransverseMercator::latLonToNorthingEasting(const double &latitude,
...
MSP::CCS::GeodeticCoordinates *TransverseMercator::convertToGeodetic(
  ...
  if ((easting < (TranMerc_False_Easting - TranMerc_Delta_Easting)) ||
      (easting > (TranMerc_False_Easting +
                  TranMerc_Delta_Easting))) {  // easting out of range
    throw CoordinateConversionException(ErrorMessages::easting);
  }

  if ((northing < (TranMerc_False_Northing - TranMerc_Delta_Northing)) ||
      (northing > (TranMerc_False_Northing +
                   TranMerc_Delta_Northing))) {  // northing out of range
...
void TransverseMercator::northingEastingToLatLon(const double &northing,
...
void TransverseMercator::generateCoefficients(double invfla, double &n1,
...
*/
  ...
  if ((strcmp(ellipsoidCode, "AA") == 0) ||
      (strcmp(ellipsoidCode, "AM") == 0)) {
  ...
  } else if ((strcmp(ellipsoidCode, "EA") == 0) ||
             (strcmp(ellipsoidCode, "EB") == 0) ||
             (strcmp(ellipsoidCode, "EC") == 0) ||
             (strcmp(ellipsoidCode, "ED") == 0) ||
  ...
  } else if ((strcmp(ellipsoidCode, "BN") == 0) ||
             (strcmp(ellipsoidCode, "BR") == 0)) {
  ...
  } else if ((strcmp(ellipsoidCode, "KA") == 0) ||
             (strcmp(ellipsoidCode, "HE") == 0) ||
  ...
  } else if ((strcmp(ellipsoidCode, "SA") == 0) ||
             (strcmp(ellipsoidCode, "AN") == 0)) {
  ...
  } else if ((strcmp(ellipsoidCode, "IN") == 0) ||
             (strcmp(ellipsoidCode, "HO") == 0)) {
...
void TransverseMercator::checkLatLon(double latitude, double deltaLon) {
  ...
  if (delta < testAngle) testAngle = delta;
...
double TransverseMercator::geodeticLat(double sinChi, double e) {
  double p;
  double pSq;
  double s_old = 1.0e99;
  double s = sinChi;
...
void TransverseMercator::computeHyperbolicSeries(double twoX, double c2kx[],
                                                 double s2kx[]) {
...
void TransverseMercator::computeTrigSeries(double twoY, double c2ky[],
                                           double s2ky[]) {
  // Use trig identities to compute
