 ...
 *(latitude, longitude in radians and height in meters) and Local Cartesian
 *coordinates (X, Y, Z).
 *
 * ERROR HANDLING
 *
 ...
 *									                    (250
 *to 350) LOCCART_ORIGIN_LAT_ERROR    : Origin Latitude out of valid range
 *                                      (-90 to 90 degrees)
 *      LOCCART_ORIGIN_LON_ERROR    : Origin Longitude out of valid range
 *                                      (-180 to 360 degrees)
 *		  LOCCART_ORIENTATION_ERROR   : Orientation angle out of valid
 *range
 *									                    (-360
 *to 360 degrees)
 *
...
LocalCartesian::LocalCartesian(double ellipsoidSemiMajorAxis,
   ...
   * and local origin parameters as inputs and sets the corresponding state
   * variables.
   *
   *    ellipsoidSemiMajorAxis : Semi-major axis of ellipsoid, in meters (input)
   *    ellipsoidFlattening    : Flattening of ellipsoid (input) originLongitude
   * : Longitude of the local origin, in radians (input) originLatitude :
   * Latitude of the local origin, in radians  (input) originHeight   :
   * Ellipsoid height of the local origin, in meters   (input) orientation    :
   * Orientation angle of the local cartesian coordinate system, in radians
   * (input)
   */
  ...
  if ((originLatitude < -PI_OVER_2) ||
      (originLatitude > PI_OVER_2)) { /* origin latitude out of range */
    throw CoordinateConversionException(ErrorMessages::originLatitude);
  }
  if ((originLongitude < -PI) ||
      (originLongitude > TWO_PI)) { /* origin longitude out of range */
    throw CoordinateConversionException(ErrorMessages::originLongitude);
  }
  if ((orientation < -PI) ||
      (orientation > TWO_PI)) { /* orientation angle out of range */
...
LocalCartesianParameters* LocalCartesian::getParameters() const {
   ...
   * (output) originLongitude           : Longitude of the local origin, in
   * radians         (output) originLatitude            : Latitude of the local
   * origin, in radians          (output) originHeight              : Ellipsoid
   * height of the local origin, in meters   (output) orientation : Orientation
   * angle of the local cartesian coordinate system, in radians (output)
   */

  return new LocalCartesianParameters(
      CoordinateType::localCartesian, LocalCart_Origin_Long,
      LocalCart_Origin_Lat, LocalCart_Origin_Height, LocalCart_Orientation);
}

MSP::CCS::CartesianCoordinates* LocalCartesian::convertFromGeodetic(
  ...
  if ((longitude < -PI) ||
      (longitude > TWO_PI)) { /* geodetic longitude out of range */
  ...
  try {
    geocentricCoordinates =
        geocentric->convertFromGeodetic(geodeticCoordinates);
...
MSP::CCS::GeodeticCoordinates* LocalCartesian::convertToGeodetic(
   ...
   * and height), according to the current ellipsoid and local origin
   * parameters.
   *
  ...
  try {
    ...
    if (longitude > PI) geodeticCoordinates->setLongitude(longitude -= TWO_PI);
...
MSP::CCS::CartesianCoordinates* LocalCartesian::convertFromGeocentric(
  ...
  if (LocalCart_Orientation == 0.0) {
    ...
    X = (-Cos_LocalCart_Orientation * Sin_LocalCart_Origin_Lon +
         Sin_Lat_Sin_Orient * Cos_LocalCart_Origin_Lon) *
            u_MINUS_u0 +
        (Cos_LocalCart_Orientation * Cos_LocalCart_Origin_Lon +
         Sin_Lat_Sin_Orient * Sin_LocalCart_Origin_Lon) *
            v_MINUS_v0 +
        (-Sin_LocalCart_Orientation * cos_lat_w_MINUS_w0);

    Y = (-Sin_LocalCart_Orientation * Sin_LocalCart_Origin_Lon -
         Sin_Lat_Cos_Orient * Cos_LocalCart_Origin_Lon) *
            u_MINUS_u0 +
        (Sin_LocalCart_Orientation * Cos_LocalCart_Origin_Lon -
         Sin_Lat_Cos_Orient * Sin_LocalCart_Origin_Lon) *
            v_MINUS_v0 +
...
MSP::CCS::CartesianCoordinates* LocalCartesian::convertToGeocentric(
    const MSP::CCS::CartesianCoordinates* cartesianCoordinates) {
  /*
  ...
  if (LocalCart_Orientation == 0.0) {
    ...
    V = Cos_LocalCart_Origin_Lon * X - sin_lat_y * Sin_LocalCart_Origin_Lon +
        cos_lat_z * Sin_LocalCart_Origin_Lon + v0;
    ...
    V = Cos_LocalCart_Origin_Lon * rotated_x -
        Sin_LocalCart_Origin_Lon * rotated_y_sin_lat +
        Sin_LocalCart_Origin_Lon * z_cos_lat + v0;
    W = Cos_LocalCart_Origin_Lat * rotated_y + Sin_LocalCart_Origin_Lat * Z +
        w0;
  }
