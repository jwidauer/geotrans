...
const double NZMG_Origin_Long =
    (173.0 * PI / 180.0); /* Longitude of origin, in radians */
...
double A[] = {0.6399175073, -0.1358797613, 0.063294409, -0.02526853, 0.0117879,
              -0.0055161,   0.0026906,     -0.001333,   0.00067,     -0.00034};

Complex B[] = {{0.7557853228, 0.0},         {0.249204646, 0.003371507},
               {-0.001541739, 0.041058560}, {-0.10162907, 0.01727609},
               {-0.26623489, -0.36249218},  {-0.6870983, -1.1651967}};

Complex C[] = {{1.3231270439, 0.0},         {-0.577245789, -0.007809598},
               {0.508307513, -0.112208952}, {-0.15094762, 0.18200602},
               {1.01418179, 1.64497696},    {1.9660549, 2.5127645}};

double D[] = {1.5627014243, 0.5185406398, -0.03333098, -0.1052906, -0.0368594,
...
NZMG::NZMG(char* ellipsoidCode) : CoordinateSystem(6378388.0, 1 / 297.0) {
  /*
  ...
  if (strcmp(ellipsoidCode, International) !=
      0) { /* Ellipsoid must be International */
...
EllipsoidParameters* NZMG::getParameters() const {
  ...
  return new EllipsoidParameters(semiMajorAxis, flattening,
                                 (char*)NZMGEllipsoidCode);
}

MSP::CCS::MapProjectionCoordinates* NZMG::convertFromGeodetic(
...
MSP::CCS::GeodeticCoordinates* NZMG::convertToGeodetic(
  ...
  if ((easting < NZMG_Min_Easting) ||
      (easting > NZMG_Max_Easting)) { /* Easting out of range  */
    throw CoordinateConversionException(ErrorMessages::easting);
  }
  if ((northing < NZMG_Min_Northing) ||
      (northing > NZMG_Max_Northing)) { /* Northing out of range */
    throw CoordinateConversionException(ErrorMessages::northing);
  }
  ...
  dphi = D[8];
  for (n = 7; n >= 0; n--) dphi = dphi * Zeta.real + D[n];
  dphi *= Zeta.real;
  ...
  if ((longitude > PI) && (longitude - PI < 1.0e-6)) longitude = PI;
