 ...
 *    08/13/12          S. Gillis, MSP_00029654, Added lat/lon to
 * define7ParamDatum
 */
...
GeodeticCoordinates* molodenskyShift(const double a, const double da,
                                     const double f, const double df,
   ...
   *    targetLatitude  : Calculated latitude in radians. (output) targetHeight
   * : Calculated height in meters.                   (output)
   */
  ...
  if (targetLongitude > TWO_PI) targetLongitude -= TWO_PI;
  if (targetLongitude < (-PI)) targetLongitude += TWO_PI;

  return new GeodeticCoordinates(CoordinateType::geodetic, targetLongitude,
                                 targetLatitude, targetHeight);
}
...
DatumLibraryImplementation* DatumLibraryImplementation::getInstance() {
  ...
  if (instance == 0) instance = new DatumLibraryImplementation;
...
void DatumLibraryImplementation::define3ParamDatum(
    const char* code, const char* name, const char* ellipsoidCode,
    double deltaX, double deltaY, double deltaZ, double sigmaX, double sigmaY,
    double sigmaZ, double westLongitude, double eastLongitude,
    double southLatitude, double northLatitude) {
  /*
   * The function define3ParamDatum creates a new local 3-parameter datum with
   * the specified code, name, and axes.  If the datum table has not been
   * initialized, the specified code is already in use, or a new version of the
   * 3-param.dat file cannot be created, an exception is thrown. Note that the
   * indexes of all datums in the datum table may be changed by this function.
   *
  ...
  if (_ellipsoidLibraryImplementation) {
    _ellipsoidLibraryImplementation->ellipsoidIndex(ellipsoidCode,
                                                    &ellipsoid_index);
  } else
  ...
  datumList.push_back(new ThreeParameterDatum(
      numDatums, (char*)datum_Code, (char*)ellipsoidCode, (char*)name,
      DatumType::threeParamDatum, deltaX, deltaY, deltaZ, westLongitude,
      eastLongitude, southLatitude, northLatitude, sigmaX, sigmaY, sigmaZ,
      true));
  datum3ParamCount++;
...
void DatumLibraryImplementation::define7ParamDatum(
    const char* code, const char* name, const char* ellipsoidCode,
    double deltaX, double deltaY, double deltaZ, double rotationX,
    double rotationY, double rotationZ, double scale, double westLongitude,
    double eastLongitude, double southLatitude, double northLatitude) {
  /*
   * The function define7ParamDatum creates a new local 7-parameter datum with
   * the specified code, name, and axes.  If the datum table has not been
   * initialized, the specified code is already in use, or a new version of the
   * 7-param.dat file cannot be created, an exception is thrown. Note that the
   * indexes of all datums in the datum table may be changed by this function.
   *
   *   code          : 5-letter new datum code.                      (input)
   *   name          : Name of the new datum                         (input)
  ...
  datumList.insert(
      datumList.begin() + MAX_WGS + datum7ParamCount,
      new SevenParameterDatum(
          datum7ParamCount, (char*)datum_Code, (char*)ellipsoidCode,
          (char*)name, DatumType::sevenParamDatum, deltaX, deltaY, deltaZ,
          westLongitude, eastLongitude, southLatitude, northLatitude,
          rotationX / SECONDS_PER_RADIAN, rotationY / SECONDS_PER_RADIAN,
          rotationZ / SECONDS_PER_RADIAN, scale, true));
  datum7ParamCount++;
...
void DatumLibraryImplementation::datumIndex(const char* code, long* index) {
  ...
  *index = 0;
  ...
  if (length > (DATUM_CODE_LENGTH - 1))
    ...
    for (i = 0; i < length; i++) temp_code[i] = (char)toupper(temp_code[i]);

    /* Strip blank spaces */
    while (pos < length) {
      if (isspace(temp_code[pos])) {
        for (i = pos; i <= length; i++) temp_code[i] = temp_code[i + 1];
        length -= 1;
      } else
        pos += 1;
    }
...
Accuracy* DatumLibraryImplementation::datumShiftError(
    const long sourceIndex, const long targetIndex, double longitude,
    double latitude, Accuracy* sourceAccuracy, Precision::Enum precision) {
  ...
  if (sourceIndex == targetIndex) { /* Just copy */
        ...
        if ((sourceThreeParameterDatum->sigmaX() < 0) ||
          ...
          se90_in = 2.5003 *
                    (sourceThreeParameterDatum->sigmaX() +
                     ...
                     sourceThreeParameterDatum->sigmaZ()) /
                    3.0;
        }
        break;
      }
        ...
        if ((targetThreeParameterDatum->sigmaX() < 0) ||
          ...
          se90_out = 2.5003 *
                     (targetThreeParameterDatum->sigmaX() +
                      ...
                      targetThreeParameterDatum->sigmaZ()) /
                     3.0;
        }
        break;
      }
    ...
    if ((circularError90 < 0.0) || (ce90_in < 0.0) || (ce90_out < 0.0)) {
      ...
      if ((linearError90 < 0.0) || (le90_in < 0.0) || (le90_out < 0.0)) {
        ...
        if ((sphericalError90 < 0.0) || (se90_in < 0.0) || (se90_out < 0.0))
          ...
          sphericalError90 = sqrt((sphericalError90 * sphericalError90) +
  ...
  if (linearError90 > 0.0) {
    ...
    linearError90 = sqrt(linearError90 * linearError90 + lePrec * lePrec);
  }

  // For circular error
  if (circularError90 > 0.0) {
    double cePrec = 2.146 * sigma;
    circularError90 = sqrt(circularError90 * circularError90 + cePrec * cePrec);
  }
  ...
  if (sphericalError90 > 0.0) {
    ...
    sphericalError90 =
        sqrt(sphericalError90 * sphericalError90 + sePrec * sePrec);
  }
...
CartesianCoordinates* DatumLibraryImplementation::geocentricDatumShift(
    const long sourceIndex, const double sourceX, const double sourceY,
    const double sourceZ, const long targetIndex) {
  /*
   *  The function geocentricDatumShift shifts a geocentric coordinate (X, Y, Z
   * in meters) relative to the source datum to geocentric coordinate (X, Y, Z
   * in meters) relative to the destination datum.
   *
  ...
  if (sourceIndex == targetIndex) {
    return new CartesianCoordinates(CoordinateType::geocentric, sourceX,
                                    sourceY, sourceZ);
  } else {
    CartesianCoordinates* wgs84CartesianCoordinates =
        geocentricShiftToWGS84(sourceIndex, sourceX, sourceY, sourceZ);
...
CartesianCoordinates* DatumLibraryImplementation::geocentricShiftFromWGS84(
    const double WGS84X, const double WGS84Y, const double WGS84Z,
    const long targetIndex) {
  /*
   *  The function geocentricShiftFromWGS84 shifts a geocentric coordinate (X,
   * Y, Z in meters) relative to WGS84 to a geocentric coordinate (X, Y, Z in
   * meters) relative to the local datum referenced by index.
   *
   *  WGS84X        : X coordinate relative to WGS84 (input) WGS84Y        : Y
   * coordinate relative to WGS84                      (input) WGS84Z        : Z
   * coordinate relative to WGS84                      (input) targetIndex   :
   * Index of destination datum                          (input) targetX       :
   * X coordinate relative to the destination datum      (output) targetY : Y
   * coordinate relative to the destination datum      (output) targetZ       :
   * Z coordinate relative to the destination datum      (output)
   */
  ...
  switch (localDatum->datumType()) {
    ...
    case DatumType::wgs84Datum: {
      return new CartesianCoordinates(CoordinateType::geocentric, WGS84X,
                                      WGS84Y, WGS84Z);
    }
    case DatumType::sevenParamDatum: {
      ...
      double targetX = WGS84X - sevenParameterDatum->deltaX() -
                       sevenParameterDatum->rotationZ() * WGS84Y +
                       sevenParameterDatum->rotationY() * WGS84Z -
                       sevenParameterDatum->scaleFactor() * WGS84X;

      double targetY = WGS84Y - sevenParameterDatum->deltaY() +
                       sevenParameterDatum->rotationZ() * WGS84X -
                       sevenParameterDatum->rotationX() * WGS84Z -
                       sevenParameterDatum->scaleFactor() * WGS84Y;

      double targetZ = WGS84Z - sevenParameterDatum->deltaZ() -
                       sevenParameterDatum->rotationY() * WGS84X +
                       sevenParameterDatum->rotationX() * WGS84Y -
                       sevenParameterDatum->scaleFactor() * WGS84Z;

      return new CartesianCoordinates(CoordinateType::geocentric, targetX,
                                      targetY, targetZ);
    }
    case DatumType::threeParamDatum: {
      ThreeParameterDatum* threeParameterDatum =
          (ThreeParameterDatum*)localDatum;
...
CartesianCoordinates* DatumLibraryImplementation::geocentricShiftToWGS84(
  ...
  /*
   *  The function geocentricShiftToWGS84 shifts a geocentric coordinate (X, Y,
   * Z in meters) relative to the datum referenced by index to a geocentric
   * coordinate (X, Y, Z in meters) relative to WGS84.
   *
   *  sourceIndex : Index of source datum                         (input)
  ...
  switch (localDatum->datumType()) {
    ...
    case DatumType::sevenParamDatum: {
      ...
      double wgs84X = sourceX + sevenParameterDatum->deltaX() +
                      sevenParameterDatum->rotationZ() * sourceY -
                      sevenParameterDatum->rotationY() * sourceZ +
                      sevenParameterDatum->scaleFactor() * sourceX;

      double wgs84Y = sourceY + sevenParameterDatum->deltaY() -
                      sevenParameterDatum->rotationZ() * sourceX +
                      sevenParameterDatum->rotationX() * sourceZ +
                      sevenParameterDatum->scaleFactor() * sourceY;

      double wgs84Z = sourceZ + sevenParameterDatum->deltaZ() +
                      sevenParameterDatum->rotationY() * sourceX -
                      sevenParameterDatum->rotationX() * sourceY +
                      sevenParameterDatum->scaleFactor() * sourceZ;
...
GeodeticCoordinates* DatumLibraryImplementation::geodeticDatumShift(
    ...
    const long targetIndex) {
  /*
   *  The function geodeticDatumShift shifts geodetic coordinates (latitude,
   * longitude in radians and height in meters) relative to the source datum to
   * geodetic coordinates (latitude, longitude in radians and height in meters)
   * relative to the destination datum.
   *
   *  sourceIndex     : Index of source datum                            (input)
   ...
   *  targetIndex     : Index of destination datum                       (input)
   *  targetLongitude : Longitude in radians relative to destination
   * datum(output) targetLatitude  : Latitude in radians relative to destination
   * datum (output) targetHeight    : Height in meters relative to destination
   * datum    (output)
   */
  ...
  if (sourceIndex == targetIndex) { /* Just copy */
    return new GeodeticCoordinates(CoordinateType::geodetic, sourceLatitude,
                                   sourceLongitude, sourceHeight);
  } else {
    if (_ellipsoidLibraryImplementation) {
      if (sourceDatum->datumType() == DatumType::sevenParamDatum) {
        ...
        if (targetDatum->datumType() ==
            DatumType::sevenParamDatum) { /* Use 3-step method for both stages
                                           */
          CartesianCoordinates* targetCartesianCoordinates =
              geocentricDatumShift(sourceIndex, sourceCartesianCoordinates->x(),
                                   sourceCartesianCoordinates->y(),
                                   sourceCartesianCoordinates->z(),
                                   targetIndex);
          ...
          GeodeticCoordinates* targetGeodeticCoordinates =
              geocentricToGeodetic.convertToGeodetic(
                  targetCartesianCoordinates);

          delete targetCartesianCoordinates;
          delete sourceCartesianCoordinates;

          return targetGeodeticCoordinates;
        } else { /* Use 3-step method for 1st stage, Molodensky if possible for
                    2nd stage */
          CartesianCoordinates* wgs84CartesianCoordinates =
              geocentricShiftToWGS84(sourceIndex,
                                     sourceCartesianCoordinates->x(),
                                     sourceCartesianCoordinates->y(),
                                     sourceCartesianCoordinates->z());

          long wgs84EllipsoidIndex;
          _ellipsoidLibraryImplementation->ellipsoidIndex("WE",
                                                          &wgs84EllipsoidIndex);
      ...
      } else if (targetDatum->datumType() ==
                 DatumType::sevenParamDatum) { /* Use Molodensky if possible for
                                                  1st stage, 3-step method for
                                                  2nd stage */
        GeodeticCoordinates* wgs84GeodeticCoordinates =
            geodeticShiftToWGS84(sourceIndex, sourceCoordinates);
        ...
        Geocentric geocentricFromGeodetic(a, f);
        CartesianCoordinates* wgs84CartesianCoordinates =
            geocentricFromGeodetic.convertFromGeodetic(
                wgs84GeodeticCoordinates);
        ...
        return targetGeodeticCoordinates;
      } else { /* Use Molodensky if possible for both stages */
        GeodeticCoordinates* wgs84GeodeticCoordinates =
            geodeticShiftToWGS84(sourceIndex, sourceCoordinates);
...
GeodeticCoordinates* DatumLibraryImplementation::geodeticShiftFromWGS84(
   ...
   *    targetIndex     : Index of destination datum (input) targetLongitude :
   * Longitude (rad) relative to destination datum   (output) targetLatitude  :
   * Latitude (rad) relative to destination datum    (output) targetHeight    :
   * Height in meters relative to destination datum  (output)
   *
   */
  ...
  switch (localDatum->datumType()) {
    case DatumType::wgs72Datum: {
      GeodeticCoordinates* targetGeodeticCoordinates =
          geodeticShiftWGS84ToWGS72(WGS84Longitude, WGS84Latitude, WGS84Height);
      return targetGeodeticCoordinates;
    }
    case DatumType::wgs84Datum: {
      return new GeodeticCoordinates(CoordinateType::geodetic, WGS84Longitude,
                                     WGS84Latitude, WGS84Height);
    }
    case DatumType::sevenParamDatum:
    case DatumType::threeParamDatum: {
      if (_ellipsoidLibraryImplementation) {
        _ellipsoidLibraryImplementation->ellipsoidIndex(
            localDatum->ellipsoidCode(), &E_Index);
        _ellipsoidLibraryImplementation->ellipsoidParameters(E_Index, &a, &f);
        ...
        _ellipsoidLibraryImplementation->ellipsoidIndex("WE",
                                                        &wgs84EllipsoidIndex);
        _ellipsoidLibraryImplementation->ellipsoidParameters(
            wgs84EllipsoidIndex, &WGS84_a, &WGS84_f);

        if ((localDatum->datumType() == DatumType::sevenParamDatum) ||
            (WGS84Latitude < (-MOLODENSKY_MAX)) ||
            (WGS84Latitude > MOLODENSKY_MAX)) { /* Use 3-step method */
          Geocentric geocentricFromGeodetic(WGS84_a, WGS84_f);
          CartesianCoordinates* wgs84CartesianCoordinates =
              geocentricFromGeodetic.convertFromGeodetic(sourceCoordinates);

          CartesianCoordinates* localCartesianCoordinates =
              geocentricShiftFromWGS84(wgs84CartesianCoordinates->x(),
                                       wgs84CartesianCoordinates->y(),
                                       wgs84CartesianCoordinates->z(),
                                       targetIndex);

          Geocentric geocentricToGeodetic(a, f);
          GeodeticCoordinates* targetGeodeticCoordinates =
              geocentricToGeodetic.convertToGeodetic(localCartesianCoordinates);

          delete localCartesianCoordinates;
          delete wgs84CartesianCoordinates;

          return targetGeodeticCoordinates;
        } else { /* Use Molodensky's method */
          da = a - WGS84_a;
          df = f - WGS84_f;
          ...
          GeodeticCoordinates* targetGeodeticCoordinates =
              molodenskyShift(WGS84_a, da, WGS84_f, df, dx, dy, dz,
...
GeodeticCoordinates* DatumLibraryImplementation::geodeticShiftToWGS84(
  ...
  Datum* localDatum = datumList[sourceIndex];
  switch (localDatum->datumType()) {
    case DatumType::wgs72Datum: { /* Special case for WGS72 */
      return geodeticShiftWGS72ToWGS84(sourceLongitude, sourceLatitude,
                                       sourceHeight);
    }
    case DatumType::wgs84Datum: { /* Just  copy */
      return new GeodeticCoordinates(CoordinateType::geodetic, sourceLongitude,
                                     sourceLatitude, sourceHeight);
    }
    case DatumType::sevenParamDatum:
    case DatumType::threeParamDatum: {
      if (_ellipsoidLibraryImplementation) {
        ...
        if ((localDatum->datumType() == DatumType::sevenParamDatum) ||
            ...
            (sourceLatitude > MOLODENSKY_MAX)) { /* Use 3-step method */
          Geocentric geocentricFromGeodetic(a, f);
          CartesianCoordinates* localCartesianCoordinates =
              geocentricFromGeodetic.convertFromGeodetic(sourceCoordinates);
          ...
          CartesianCoordinates* wgs84CartesianCoordinates =
              geocentricShiftToWGS84(sourceIndex,
                                     localCartesianCoordinates->x(),
                                     localCartesianCoordinates->y(),
                                     localCartesianCoordinates->z());
          ...
          GeodeticCoordinates* wgs84GeodeticCoordinates =
              molodenskyShift(a, da, f, df, dx, dy, dz, sourceLongitude,
                              sourceLatitude, sourceHeight);
...
void DatumLibraryImplementation::validDatum(const long index, double longitude,
   ...
   * within the validity rectangle for the specified datum.  It returns zero if
   * the specified location is NOT within the validity rectangle, and returns 1
   * otherwise.
   *
   *   index     : The index of a given datum in the datum table      (input)
  ...
  *result = 0;
  ...
  /* The west and east longitudes may be in the range 0 to 360
     ...
     If west and east are in the -180 to 180 range and west > east, put them in
     the 0 to 360 range and adjust longitude if necessary. If west and east are
     in the 0 to 360 range and west > east, put them in the -180 to 180 range.
     If west < east, adjust longitude to the 0 to 360 range
  */
  if ((west_longitude < 0) || (east_longitude < 0)) {
    if (west_longitude > east_longitude) {
      if (west_longitude < 0) west_longitude += TWO_PI;
      if (east_longitude < 0) east_longitude += TWO_PI;
      if (longitude < 0) longitude += TWO_PI;
    }
  } else if ((west_longitude > PI) || (east_longitude > PI)) {
    if (west_longitude > east_longitude) {
      if (west_longitude > PI) west_longitude -= TWO_PI;
      if (east_longitude > PI) east_longitude -= TWO_PI;
    } else {
      if (longitude < 0) longitude += TWO_PI;
    }
  }
  ...
  if ((datum->southLatitude() <= latitude) &&
      (latitude <= datum->northLatitude()) && (west_longitude <= longitude) &&
...
void DatumLibraryImplementation::setEllipsoidLibraryImplementation(
  ...
  /*
   *  The function setEllipsoidLibraryImplementation sets the ellipsoid library
   * information which is needed to create datums and calculate datum shifts.
   *
   *   __ellipsoidLibraryImplementation  : Ellipsoid library
...
void DatumLibraryImplementation::loadDatums() {
  ...
  while (!feof(fp_7param)) {
    ...
    if (fscanf(fp_7param, "%s ", code) <= 0) {
      ...
      if (code[0] == '*') {
        userDefined = true;
        for (int i = 0; i < DATUM_CODE_LENGTH; i++) code[i] = code[i + 1];
      }
    }

    char name[DATUM_NAME_LENGTH];
    if (fscanf(fp_7param, "\"%32[^\"]\"", name) <= 0) name[0] = '\0';
    ...
    if (fscanf(fp_7param, " %s %lf %lf %lf %lf %lf %lf %lf ", ellipsoidCode,
               &deltaX, &deltaY, &deltaZ, &rotationX, &rotationY, &rotationZ,
               &scaleFactor) <= 0) {
      fclose(fp_7param);
      throw CoordinateConversionException(ErrorMessages::datumFileParseError);
    } else { /* convert from degrees to radians */
      ...
      datumList.push_back(new SevenParameterDatum(
          index, code, ellipsoidCode, name, DatumType::sevenParamDatum, deltaX,
          deltaY, deltaZ, -PI, +PI, -PI / 2.0, +PI / 2.0, rotationX, rotationY,
          rotationZ, scaleFactor, userDefined));
    }
    index++;
    datum7ParamCount++;
  }
  ...
  while (!feof(fp_3param)) {
    ...
    char name[DATUM_NAME_LENGTH];
    if (fscanf(fp_3param, "\"%32[^\"]\"", name) <= 0) name[0] = '\0';
    ...
    if (fscanf(fp_3param, " %s %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf ",
               ellipsoidCode, &deltaX, &sigmaX, &deltaY, &sigmaY, &deltaZ,
               &sigmaZ, &southLatitude, &northLatitude, &westLongitude,
               &eastLongitude) <= 0) {
      fclose(fp_3param);
      throw CoordinateConversionException(ErrorMessages::datumFileParseError);
    } else {
      ...
      datumList.push_back(new ThreeParameterDatum(
          index, code, ellipsoidCode, name, DatumType::threeParamDatum, deltaX,
          deltaY, deltaZ, westLongitude, eastLongitude, southLatitude,
          northLatitude, sigmaX, sigmaY, sigmaZ, userDefined));
    }
...
void DatumLibraryImplementation::write3ParamFile() {
  /*
   *  The function write3ParamFile writes the 3 parameter datums in the datum
   * list to the 3_param.dat file.
   */
  ...
  if ((fp_3param = fopen(FileName, "w")) == NULL) { /* fatal error */
    char message[256] = "";
    if (NULL == PathName) {
      strcpy(message, "Environment variable undefined: MSPCCS_DATA.");
    } else {
  ...
  while (index < size) {
    ThreeParameterDatum* _3parameterDatum =
        (ThreeParameterDatum*)datumList[index];
    if (_3parameterDatum) {
      strcpy(datum_name, "\"");
      strcat(datum_name, datumList[index]->name());
      strcat(datum_name, "\"");
      if (_3parameterDatum->userDefined()) fprintf(fp_3param, "*");

      fprintf(fp_3param,
              "%-6s  %-33s%-2s %4.0f %4.0f %4.0f %4.0f %5.0f %4.0f %4.0f %4.0f "
              "%4.0f %4.0f \n",
              _3parameterDatum->code(), datum_name,
              _3parameterDatum->ellipsoidCode(), _3parameterDatum->deltaX(),
              _3parameterDatum->sigmaX(), _3parameterDatum->deltaY(),
              _3parameterDatum->sigmaY(), _3parameterDatum->deltaZ(),
...
void DatumLibraryImplementation::write7ParamFile() {
  /*
   *  The function write3ParamFile writes the 7 parameter datums in the datum
   * list to the 7_param.dat file.
   */
  ...
  if ((fp_7param = fopen(FileName, "w")) == NULL) { /* fatal error */
    char message[256] = "";
    if (NULL == PathName) {
      strcpy(message, "Environment variable undefined: MSPCCS_DATA.");
    } else {
  ...
  while (index < endIndex) {
    SevenParameterDatum* _7parameterDatum =
        (SevenParameterDatum*)datumList[index];
    if (_7parameterDatum) {
      strcpy(datum_name, "\"");
      strcat(datum_name, datumList[index]->name());
      strcat(datum_name, "\"");
      if (_7parameterDatum->userDefined()) fprintf(fp_7param, "*");

      fprintf(fp_7param,
              "%-6s  %-33s%-2s  %4.0f  %4.0f  %4.0f % 4.3f % 4.3f % 4.3f   % "
              "4.10f \n",
              _7parameterDatum->code(), datum_name,
              _7parameterDatum->ellipsoidCode(), _7parameterDatum->deltaX(),
              _7parameterDatum->deltaY(), _7parameterDatum->deltaZ(),
...
GeodeticCoordinates* DatumLibraryImplementation::geodeticShiftWGS84ToWGS72(
    const double WGS84Longitude, const double WGS84Latitude,
    const double WGS84Height) {
  /*
   *  The function geodeticShiftWGS84ToWGS72 shifts a geodetic coordinate
   * (latitude, longitude in radians and height in meters) relative to WGS84 to
   * a geodetic coordinate (latitude, longitude in radians and height in meters)
   * relative to WGS72.
   *
   *  WGS84Longitude : Longitude in radians relative to WGS84    (input)
   *  WGS84Latitude  : Latitude in radians relative to WGS84     (input)
  ...
  _ellipsoidLibraryImplementation->ellipsoidParameters(wgs84EllipsoidIndex,
                                                       &WGS84_a, &WGS84_f);

  long wgs72EllipsoidIndex;
  _ellipsoidLibraryImplementation->ellipsoidIndex("WD", &wgs72EllipsoidIndex);
  _ellipsoidLibraryImplementation->ellipsoidParameters(wgs72EllipsoidIndex,
                                                       &WGS72_a, &WGS72_f);
  ...
  if (wgs72Longitude > PI) wgs72Longitude -= TWO_PI;
  if (wgs72Longitude < -PI) wgs72Longitude += TWO_PI;

  return new GeodeticCoordinates(CoordinateType::geodetic, wgs72Longitude,
                                 wgs72Latitude, wgs72Height);
}

GeodeticCoordinates* DatumLibraryImplementation::geodeticShiftWGS72ToWGS84(
    const double WGS72Longitude, const double WGS72Latitude,
    const double WGS72Height) {
  /*
   *  The function geodeticShiftWGS72ToWGS84 shifts a geodetic coordinate
   * (latitude, longitude in radians and height in meters) relative to WGS72 to
   * a geodetic coordinate (latitude, longitude in radians and height in meters)
   * relative to WGS84.
   *
  ...
  Delta_Lat = (4.5 * cos(WGS72Latitude)) / (WGS72_a * Q) +
              (df * sin(2 * WGS72Latitude)) / Q;
  Delta_Lat /= SECONDS_PER_RADIAN;
  Delta_Lon = 0.554 / SECONDS_PER_RADIAN;
...
CartesianCoordinates* DatumLibraryImplementation::geocentricShiftWGS84ToWGS72(
    const double X_WGS84, const double Y_WGS84, const double Z_WGS84) {
  /*
   *  The function geocentricShiftWGS84ToWGS72 shifts a geocentric coordinate
   * (X, Y, Z in meters) relative to WGS84 to a geocentric coordinate (X, Y, Z
   * in meters) relative to WGS72.
   *
  ...
  GeodeticCoordinates* wgs84GeodeticCoordinates =
      geocentric84.convertToGeodetic(new CartesianCoordinates(
          CoordinateType::geocentric, X_WGS84, Y_WGS84, Z_WGS84));

  GeodeticCoordinates* wgs72GeodeticCoordinates = geodeticShiftWGS84ToWGS72(
      wgs84GeodeticCoordinates->longitude(),
      wgs84GeodeticCoordinates->latitude(), wgs84GeodeticCoordinates->height());
  ...
  CartesianCoordinates* wgs72GeocentricCoordinates =
      geocentric72.convertFromGeodetic(wgs72GeodeticCoordinates);
...
CartesianCoordinates* DatumLibraryImplementation::geocentricShiftWGS72ToWGS84(
    const double X, const double Y, const double Z) {
  /*
   *  The function geocentricShiftWGS72ToWGS84 shifts a geocentric coordinate
   * (X, Y, Z in meters) relative to WGS72 to a geocentric coordinate (X, Y, Z
   * in meters) relative to WGS84.
   *
  ...
  Geocentric geocentric72(a_72, f_72);
  GeodeticCoordinates* wgs72GeodeticCoordinates =
      geocentric72.convertToGeodetic(
          new CartesianCoordinates(CoordinateType::geocentric, X, Y, Z));

  GeodeticCoordinates* wgs84GeodeticCoordinates = geodeticShiftWGS72ToWGS84(
      wgs72GeodeticCoordinates->longitude(),
      wgs72GeodeticCoordinates->latitude(), wgs72GeodeticCoordinates->height());
  ...
  Geocentric geocentric84(a_84, f_84);
  CartesianCoordinates* wgs84GeocentricCoordinates =
      geocentric84.convertFromGeodetic(wgs84GeodeticCoordinates);
