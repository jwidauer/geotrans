...
class MSP_DTCC_API DatumLibraryImplementation {
 ...
 public:
  ...
  void define3ParamDatum(const char *code, const char *name,
                         const char *ellipsoidCode, double deltaX,
                         double deltaY, double deltaZ, double sigmaX,
  ...
  void define7ParamDatum(const char *code, const char *name,
                         const char *ellipsoidCode, double deltaX,
                         double deltaY, double deltaZ, double rotationX,
                         double rotationY, double rotationZ, double scale,
  ...
  CartesianCoordinates *geocentricDatumShift(const long sourceIndex,
  ...
  CartesianCoordinates *geocentricShiftFromWGS84(const double WGS84X,
                                                 const double WGS84Y,
                                                 const double WGS84Z,
                                                 const long targetIndex);
  ...
  CartesianCoordinates *geocentricShiftToWGS84(const long sourceIndex,
                                               const double sourceX,
                                               const double sourceY,
                                               const double sourceZ);
   ...
   *
   *  sourceIndex     : Index of source datum (input) sourceLongitude :
   * Longitude in radians relative to source datum       (input) sourceLatitude
   * : Latitude in radians relative to source datum        (input) sourceHeight
   * : Height in meters relative to source datum           (input) targetIndex
   * : Index of destination datum                          (input)
   *  targetLongitude : Longitude in radians relative to destination datum
   * (output) targetLatitude  : Latitude in radians relative to destination
   * datum   (output) targetHeight    : Height in meters relative to destination
   * datum      (output)
   */

  GeodeticCoordinates *geodeticDatumShift(
   ...
   * Longitude in radians relative to destination datum (output) targetLatitude
   * : Latitude in radians relative to destination datum  (output) targetHeight
  ...
  void setEllipsoidLibraryImplementation(
 ...
 private:
  ...
  /*
   *  The function write7ParamFile writes the 7 parameter datums in the datum
  ...
  GeodeticCoordinates *geodeticShiftWGS84ToWGS72(const double WGS84Longitude,
  ...
  GeodeticCoordinates *geodeticShiftWGS72ToWGS84(const double WGS72Longitude,
  ...
  CartesianCoordinates *geocentricShiftWGS84ToWGS72(const double X_WGS84,
  ...
  CartesianCoordinates *geocentricShiftWGS72ToWGS84(const double X,
                                                    const double Y,
                                                    const double Z);
