 ...
 *    DatumLibraryImplementation.h    - for prototype error ehecking and error
 * codes
 */
...
DatumLibrary::~DatumLibrary() { _datumLibraryImplementation = 0; }
...
void DatumLibrary::defineDatum(const int datumType, const char *datumCode,
  ...
  if (datumType == DatumType::threeParamDatum) {
    _datumLibraryImplementation->define3ParamDatum(
        datumCode, datumName, ellipsoidCode, deltaX, deltaY, deltaZ, sigmaX,
        sigmaY, sigmaZ, westLongitude, eastLongitude, southLatitude,
        northLatitude);
  } else if (datumType == DatumType::sevenParamDatum) {
    _datumLibraryImplementation->define7ParamDatum(
        datumCode, datumName, ellipsoidCode, deltaX, deltaY, deltaZ, rotationX,
        rotationY, rotationZ, scaleFactor, westLongitude, eastLongitude,
...
void DatumLibrary::removeDatum(const char *code) {
   ...
   * version of the 3-param.dat file cannot be created, an error code is
   * returned, otherwise DATUM_NO_ERROR is returned.  Note that the indexes of
   * all datums in the datum table may be changed by this function.
   *
...
void DatumLibrary::getDatumInfo(const long index, char *code, char *name,
                                char *ellipsoidCode) {
  /*
...
void DatumLibrary::getDatumParameters(
    const long index, DatumType::Enum *datumType, double *deltaX,
    double *deltaY, double *deltaZ, double *sigmaX, double *sigmaY,
    double *sigmaZ, double *westLongitude, double *eastLongitude,
    double *southLatitude, double *northLatitude, double *rotationX,
    double *rotationY, double *rotationZ, double *scaleFactor) {
  /*
  ...
  _datumLibraryImplementation->datumTranslationValues(index, deltaX, deltaY,
                                                      deltaZ);
  ...
  if (*datumType == DatumType::threeParamDatum) {
    _datumLibraryImplementation->datumStandardErrors(index, sigmaX, sigmaY,
                                                     sigmaZ);
