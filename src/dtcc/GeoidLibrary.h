 ...
 *    12-17-10          RD Craig added pointer to EGM2008 interpolator
 * (BAEts26267).
 *
 */
 ...
 *  GEOID_NO_ERROR               : No errors occurred in function\n
 *  GEOID_FILE_OPEN_ERROR        : Geoid file opening error\n
 *  GEOID_INITIALIZE_ERROR       : Geoid separation database can not
 * initialize\n GEOID_LAT_ERROR              : Latitude out of valid range
 *                                 (-90 to 90 degrees)\n
 *  GEOID_LON_ERROR              : Longitude out of valid range\n
 *                                 (-180 to 360 degrees)\n
...
class MSP_DTCC_API GeoidLibrary {
 ...
 public:
  ...
  void convertEllipsoidToEGM84TenDegBilinearHeight(double longitude,
                                                   double latitude,
  ...
  void convertEllipsoidToEGM84TenDegNaturalSplineHeight(double longitude,
                                                        double latitude,
                                                        double ellipsoidHeight,
                                                        double *geoidHeight);
  ...
  void convertEllipsoidToEGM84ThirtyMinBiLinearHeight(double longitude,
                                                      double latitude,
                                                      double ellipsoidHeight,
                                                      double *geoidHeight);
   ...
   * function uses the EGM2008 gravity model, plus the bicubic spline
  ...
  void convertEllipsoidHeightToEGM2008GeoidHeight(double longitude,
                                                  double latitude,
                                                  double ellipsoidHeight,
                                                  double *geoidHeight);
  ...
  void convertEGM84TenDegBilinearToEllipsoidHeight(double longitude,
                                                   double latitude,
                                                   double geoidHeight,
                                                   double *ellipsoidHeight);
  ...
  void convertEGM84ThirtyMinBiLinearToEllipsoidHeight(double longitude,
                                                      double latitude,
                                                      double geoidHeight,
                                                      double *ellipsoidHeight);
  ...
  void convertEGM2008GeoidHeightToEllipsoidHeight(double longitude,
                                                  double latitude,
                                                  double geoidHeight,
                                                  double *ellipsoidHeight);
 ...
 private:
   ...
   * so they load quickly, and they permit high-resolution geoid separations to
   * be computed even on small computer systems.  AOI grids do not need to be
   * reloaded into computer memory if all requested geoid separations are
   * spatially localized. AOI grid reloads are automatic, and they require no
   * action by GeoidLibrary users.
   */
  ...
  void bilinearInterpolateDoubleHeights(double longitude, double latitude,
                                        double scale_factor, int num_cols,
                                        int num_rows, double *height_buffer,
                                        double *delta_height);
  ...
  void bilinearInterpolate(double longitude, double latitude,
                           double scale_factor, int num_cols, int num_rows,
                           float *height_buffer, double *delta_height);
  ...
  void naturalSplineInterpolate(double longitude, double latitude,
                                double scale_factor, int num_cols, int num_rows,
                                int max_index, float *height_buffer,
                                double *delta_height);
