...
//       geoid: these heights are properly called elevations or
//       heights-above-MSL.
...
Egm2008GeoidGrid::Egm2008GeoidGrid(void) {
  ...
  _gridFname += "/Und_min2.5x2.5_egm2008_WGS84_TideFree_reformatted.grd";
#endif
  ...
  // strcpy( fileName, "Und_min2.5x2.5_egm2008_WGS84_TideFree_reformatted.grd"
  // );
  //
...
Egm2008GeoidGrid::Egm2008GeoidGrid(const Egm2008GeoidGrid& oldGrid)  // input
{
  // November  19, 2010: Version 1.00
...
int Egm2008GeoidGrid::loadGridCoords(int i,              // input
                                     int j,              // input
                                     ...
                                     double& longitude)  // output
{
  ...
  try {
    ...
    if (i < LIMIT1) {
      latitude = (-PIDIV2 - _dLat * double(i - LIMIT1));

      k = j + (_nOrigCols / 2);

      if (k >= _nGridCols) k -= _nOrigCols;

      longitude = (_baseLongitude + _dLon * double(k));
    }

    // Indices refer to NGA grid region .....

    if ((i >= LIMIT1) && (i <= LIMIT2)) {
      latitude = (_baseLatitude + _dLat * double(i));

      longitude = (_baseLongitude + _dLon * double(j));
    }

    // Indices refer to Northern pad region .....

    if (i > LIMIT2) {
      latitude = (PIDIV2 - _dLat * double(i - LIMIT2));
...
int Egm2008GeoidGrid::initSpline(int n,                 // input
                                 ...
                                 double moments[])      // output
{
  ...
  try {
    ...
    for (k = 2; k < n; k++) {
      p = (w[k - 2] / 2.0) + 2.0;
      w[k - 1] = -0.5 / p;
      moments[k - 1] = (3.0 * (posts[k] - (2.0 * posts[k - 1]) + posts[k - 2]) -
                        (moments[k - 2] / 2.0)) /
                       p;
    }
...
double Egm2008GeoidGrid::spline(int n,                   // input
                                double x,                // input
                                const double posts[],    // input
                                const double moments[])  // input
{
  ...
  //                        (x == 0.0)        ==> x equals the first support
  //                        abscissa, (x == double(n-1) ==> x equals the final
  //                        support abscissa.
  ...
  try {
    ...
    {
      height = posts[0] +
               (x - MIN_ABSCISSA) * (posts[1] - posts[0] - (moments[1] / 6.0));
    } else if (x >= maxAbscissa)  // linear extrapolation
    {
      height = posts[n - 1] + (x - maxAbscissa) * (posts[n - 1] - posts[n - 2] +
                                                   (moments[n - 2] / 6.0));
    } else  // cubic spline interpolation
    {
      ...
      height =  // compute geoid height using Horner's rule
          posts[j] +
          dx * ((posts[j + 1] - posts[j] - (mJ / 3.0) - (mJp1 / 6.0)) +
                dx * ((mJ / 2.0) + dx * (mJp1 - mJ) / 6.0));
    }
...
void Egm2008GeoidGrid::swapBytes(void* buffer,  // input & output
                                 size_t size,   // input
                                 size_t count)  // input
{
...
int Egm2008GeoidGrid::swGridIndices(double latitude,   // input
                                    ...
                                    int& j)            // output
{
  ...
  try {
    ...
    maxLatitude = (_baseLatitude + double(_nGridRows - 1) * _dLat);

    if ((latitude < _baseLatitude) || (latitude > maxLatitude)) return (1);
