...
Egm2008FullGrid::Egm2008FullGrid(const std::string& gridFname)  // input
...
Egm2008FullGrid::Egm2008FullGrid(const Egm2008FullGrid& oldGrid)  // input
...
int Egm2008FullGrid::geoidHeight(int wSize,         // input
                                 ...
                                 double& gHeight)   // output
{
  ...
  try {
    //    MSP::CCSThreadLock  aLock( &_mutex );  // copy constructor locks
    //    thread
    ...
    for (i = 0; i < wSize; i++) {
      ...
      offset = (_nGridRows - iIndex - 1) * _nGridCols;
...
int Egm2008FullGrid::geoidHeight(double latitude,   // input
                                 double longitude,  // input
                                 double& gHeight)   // output
{
  // November 19, 2010: Version 1.00
  ...
  try {
    ...
    index = j1 + (_nGridRows - i1 - 1) * _nGridCols;
    n1 = _heightGrid[index];

    index = j2 + (_nGridRows - i2 - 1) * _nGridCols;
    n2 = _heightGrid[index];

    index = j3 + (_nGridRows - i3 - 1) * _nGridCols;
    n3 = _heightGrid[index];

    index = j4 + (_nGridRows - i4 - 1) * _nGridCols;
    ...
    lat1 = _baseLatitude + _dLat * double(i1);   // radians
    lon1 = _baseLongitude + _dLon * double(j1);  // radians
...
int Egm2008FullGrid::loadGrid(void) {
  // November 19, 2010: Version 1.00
  ...
  try {
    ...
    _heightGrid = new float[_nGridRows * _nGridCols];
    ...
    for (i = startRow; i >= endRow; i--) {
      ...
      fin.read((char*)(&_heightGrid[index0]), (_nGridCols * BYTES_PER_FLOAT));
      ...
      this->swapBytes(&_heightGrid[index0], BYTES_PER_FLOAT, _nGridCols);
