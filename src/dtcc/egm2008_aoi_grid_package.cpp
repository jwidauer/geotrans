...
const int BYTES_IN_HEADER = (3 * BYTES_PER_INT + 2 * BYTES_PER_DOUBLE);
...
Egm2008AoiGrid::Egm2008AoiGrid(const std::string& gridFname)  // input
...
Egm2008AoiGrid::Egm2008AoiGrid(const Egm2008AoiGrid& oldGrid)  // input
...
int Egm2008AoiGrid::geoidHeight(int wSize,         // input
                                ...
                                double& gHeight)   // output
{
  ...
  try {
    ...
    latIndex = double(_nGridPad) + (latitude + PIDIV2) / _dLat;
    lonIndex = double(_nGridPad) + (longitude - 0.0) / _dLon;
    ...
    if ((iMin < _minAoiRowIndex) || (iMax > _maxAoiRowIndex) ||
        (jMin < _minAoiColIndex) || (jMax > _maxAoiColIndex)) {
    ...
    if (wSize < 3) {
      status = this->geoidHeight(latitude, longitude, gHeight);

      if (status != 0) return (1);

      ; /* Normal bilinear interpolation return */
      return (0);
    }
    ...
    temp = lonIndex - double(jMin);  // 0 <= temp <= (wSize - 1)
    ...
    for (i = 0; i < wSize; i++) {
      ...
      status = this->initSpline(wSize, lonSupport, moments);

      if (status != 0) return (1);

      latSupport[i] = this->spline(wSize, temp, lonSupport, moments);
    }
    ...
    temp = latIndex - double(iMin);  // 0 <= temp <= (wSize - 1)

    // Compute moments, and interpolate final
    // geoid height at the latitude of interest .....

    status = this->initSpline(wSize, latSupport, moments);

    if (status != 0) return (1);

    gHeight = this->spline(wSize, temp, latSupport, moments);
...
int Egm2008AoiGrid::geoidHeight(double latitude,   // input
                                double longitude,  // input
                                double& gHeight)   // output
{
  // November  19, 2010: Version 1.00
  ...
  try {
    ...
    status = this->swGridIndices(latitude, longitude, i1, j1);
    ...
    index = j1 + (_nAoiRows - i1 - 1) * _nAoiCols;
    n1 = _heightGrid[index];

    index = j2 + (_nAoiRows - i2 - 1) * _nAoiCols;
    n2 = _heightGrid[index];

    index = j3 + (_nAoiRows - i3 - 1) * _nAoiCols;
    n3 = _heightGrid[index];

    index = j4 + (_nAoiRows - i4 - 1) * _nAoiCols;
    ...
    lat1 = _baseLatitude + _dLat * double(i1ww);   // radians
    lon1 = _baseLongitude + _dLon * double(j1ww);  // radians
...
int Egm2008AoiGrid::loadAoiParms(int i0,  // input
                                 int j0)  // input
{
  // November 19, 2010: Version 1.00
  ...
  try {
    ...
    status = this->loadGridCoords(i0, j0, latitude, longitude);
...
int Egm2008AoiGrid::loadGrid(void) {
  ...
  try {
    ...
    fin.open(_gridFname.c_str(), std::ifstream::binary | std::ifstream::in);
    ...
    index1 =  // Starting worldwide file location (bytes)
        BYTES_IN_HEADER +
        BYTES_PER_FLOAT *
            (_minAoiColIndex + (_nGridRows - startRow - 1) * _nGridCols);
    ...
    for (i = startRow; i >= endRow; i--) {
      ...
      fin.read((char*)(&_heightGrid[index0]), (_nAoiCols * BYTES_PER_FLOAT));
      ...
      this->swapBytes(&_heightGrid[index0], BYTES_PER_FLOAT, _nAoiCols);
...
int Egm2008AoiGrid::loadGridMetadata(void) {
  ...
  try {
    ...
    _baseLatitude = -PIDIV2 - _dLat * double(_nGridPad);  // radians
    _baseLongitude = 0.0 - _dLon * double(_nGridPad);     // radians
