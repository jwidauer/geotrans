...
const int COORD_SYS_CODE_LENGTH =
    3;  // Length of coordinate system codes (including null)
const int COORD_SYS_NAME_LENGTH =
    50;  // Max length of coordinate system names (including null)
...
const int ELLIPSOID_CODE_LENGTH =
    3;  // Length of ellipsoid codes (including null)
const int ELLIPSOID_NAME_LENGTH =
    30;  // Max length of ellipsoid names (including null)
const int CONVERT_MSG_LENGTH =
    2048;  // Max length of coordinate conversion status message
...
#endif
{
 public:
   ...
   *
   * @param[in] sourceCoordinates - Coordinates of the source coordinate system
   * to be converted
   * @param[in] sourceAccuracy - Source circular, linear and spherical errors
   * @param[out] targetCoordinates - Converted coordinates of the target
   * coordinate system
   * @param[out] targetAccuracy - Target circular, linear and spherical errors
   *
   */
  void convertSourceToTarget(CoordinateTuple* sourceCoordinates,
   ...
   *
   * @param[in] targetCoordinates - Converted coordinates of the target
   * coordinate system
   * @param[in] targetAccuracy - Target circular, linear and spherical errors
   * @param[out] sourceCoordinates - Coordinates of the source coordinate system
   * to be converted
   * @param[out] sourceAccuracy - Source circular, linear and spherical errors
   *
   */
  void convertTargetToSource(CoordinateTuple* targetCoordinates,
                             Accuracy* targetAccuracy,
   ...
   *
   * @param[in] direction - indicates whether the datum is to be used for input
   * or output
...
// compiler bug on solaris 8. Parameters needs to be public
 ...
 public:
  ...
  void copyParameters(SourceOrTarget::Enum direction,
  ...
  void setDatum(const SourceOrTarget::Enum direction, const char* index);
  ...
  void setCoordinateSystem(const SourceOrTarget::Enum direction,
  ...
  void convert(SourceOrTarget::Enum sourceDirection,
  ...
  void convertGeodeticToTarget(SourceOrTarget::Enum targetDirection,
   ...
   *
   *  sourceCoordinatesCollection  : Coordinates of the source coordinate system
   * to be converted   (input) sourceAccuracyCollection     : Source circular,
   * linear and spherical errors                  (input)
   *  targetCoordinatesCollection  : Converted coordinates of the target
   * coordinate system         (output) targetAccuracyCollection     : Target
   * circular, linear and spherical errors                  (output)
   */

  void convertCollection(
      const std::vector<MSP::CCS::CoordinateTuple*>&
          sourceCoordinatesCollection,
